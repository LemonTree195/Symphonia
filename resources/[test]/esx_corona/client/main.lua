...
RegisterNetEvent('ddx_corona:cure')
AddEventHandler('ddx_corona:cure', function()
	isInfected = false
...
RegisterNetEvent('ddx_corona:infect')
AddEventHandler('ddx_corona:infect', function()
...
RegisterNetEvent('ddx_corona:sneeze')
AddEventHandler('ddx_corona:sneeze', function(playerId)
	...
	while true do
		...
		if isInfected then
			ESX.Streaming.RequestAnimDict("timetable@gardener@smoking_joint", function()
				...
				Citizen.Wait(1400)
				TriggerServerEvent('ddx_corona:sneezeSync')
				Citizen.Wait(2600)
				local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()

				if closestPlayer ~= -1 and closestDistance < 4.0 then
					TriggerServerEvent('ddx_corona:infectPlayer', GetPlayerServerId(closestPlayer))
				end
