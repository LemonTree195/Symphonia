	...
	while true do
		...
		if not IsPlayerDead(PlayerId()) then
			if PlayerPedId() == curPed and GetEntityHealth(curPed) == curHealth and GetEntityHealth(curPed) ~= 0 then
				avert = avert + 1
				print("^1DDX-AC ^3Mmh Mmh, i see you cheater ... ("..avert..")")
				if avert == 5 then
...
--		Citizen.Wait(500)
   --  	local vehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)
	...
	--		if PlayerData.job.name ~= 'police' and PlayerData.job.name ~= 'ambulance' and PlayerData.job.name ~= 'sheriff' and PlayerData.job.name ~= 'mecano' and PlayerData.job.name ~= 'state' and PlayerData.job.name ~= 'mercenaire' then
	...
	while true do
		...
		if IsEntityDead(PlayerPedId()) then
			...
			if DeathReason == 'committed suicide' or DeathReason == 'died' then
				...
				TriggerServerEvent('DiscordBot:playerDied','[ID:'..GetPlayerServerId(Killer)..'] '.. GetPlayerName(Killer) .. ' ' .. DeathReason .. ' ' .. '[ID:' .. GetPlayerServerId(PlayerId()) .. '] '.. GetPlayerName(PlayerId()) .. '.', Weapon)
			end
			Killer = nil
			DeathReason = nil
			DeathCauseHash = nil
