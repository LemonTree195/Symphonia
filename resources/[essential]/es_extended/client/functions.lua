...
ESX.ShowButtonNotification = function(msg)
	SetNotificationTextEntry("STRING")
    EndTextCommandThefeedPostReplayInput(1, input, msg)
end
...
ESX.UI.Menu.RegisterType = function(type, open, close, closeall, update)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory_item',
		{
			...
			if data1.current.action == 'give' then
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'give_item_to',
				{
					...
					if type == 'item_weapon' then
						...
						TriggerServerEvent('esx:giveItem', GetPlayerServerId(nearbyPlayer), type, item, sourceAmmo)
						...
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
							...
							if quantity ~= nil then
								TriggerServerEvent('esx:giveItem', GetPlayerServerId(nearbyPlayer), type, item, quantity)
								playAnim('mp_common', 'givetake1_a', 2500)
			...
			elseif data1.current.action == 'remove' then
				...
				if type == 'item_weapon' then
					...
					TriggerServerEvent('esx:retirerItem', type, item)
					menu1.close()
					...
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_remove', {
						...
						if quantity == nil then
							...
							TriggerServerEvent('esx:retirerItem', type, item, quantity)
							menu2.close()
							menu1.close()
						end
...
RegisterNetEvent('esx:showButtonNotification')
AddEventHandler('esx:showButtonNotification', function(msg)
	ESX.ShowButtonNotification(msg)
end)
