...
AddEventHandler('playerSpawned', function()
	...
	if ESX.PlayerData.lastPosition ~= nil then
		...
		TriggerEvent('esx:showAdvancedStreamedNotification', 'SymphoniaLife', 'Synchronisation', 'Données du personnage chargées.', 'CHAR_FLOYD', 'symphonia', 8)
	end
...
AddEventHandler('esx:retirerItem', function(item, count)
	for i=1, #ESX.PlayerData.inventory, 1 do
		if ESX.PlayerData.inventory[i].name == item.name then
			ESX.PlayerData.inventory[i] = item
			break
...
Citizen.CreateThread(function()
	...
	Citizen.Wait(10)
	  if IsControlPressed(0, 57) then
		...
		TriggerEvent('esx:showColoredNotification', "~w~Votre ID est le " .. serverID .. "", 210)
		TriggerEvent('esx:showColoredNotification', "~w~Il y a actuellement " .. #players .. " joueur(s) connecté(s)", 210)
	  end
	end
end)
