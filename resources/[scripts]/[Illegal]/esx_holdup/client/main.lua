...
RegisterNetEvent('ddx_holdup:currentlyRobbing')
AddEventHandler('ddx_holdup:currentlyRobbing', function(currentStore)
	holdingUp, store = true, currentStore
end)

RegisterNetEvent('ddx_holdup:killBlip')
AddEventHandler('ddx_holdup:killBlip', function()
	RemoveBlip(blipRobbery)
end)

RegisterNetEvent('ddx_holdup:setBlip')
AddEventHandler('ddx_holdup:setBlip', function(position)
...
RegisterNetEvent('ddx_holdup:tooFar')
AddEventHandler('ddx_holdup:tooFar', function()
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_cancelled'))
end)

RegisterNetEvent('ddx_holdup:robberyComplete')
AddEventHandler('ddx_holdup:robberyComplete', function(award)
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_complete', award))
end)

RegisterNetEvent('ddx_holdup:startTimer')
AddEventHandler('ddx_holdup:startTimer', function()
	...
	while true do
					...
					if distance < 0.5 then
						...
						if IsControlJustReleased(0, 38) then
							if IsPedArmed(PlayerPedId(), 4) then
								TriggerServerEvent('ddx_holdup:robberyStarted', k)
							else
		...
		if holdingUp then
			...
			if Vdist(playerPos.x, playerPos.y, playerPos.z, storePos.x, storePos.y, storePos.z) > Config.MaxDistance then
				TriggerServerEvent('ddx_holdup:tooFar', store)
			end
		end
	end
end)
