...
RegisterNetEvent("ddx_bankrobbery:alertCops")
AddEventHandler("ddx_bankrobbery:alertCops", function(bankId)
    local coords = BankHeists[bankId]["Start_Robbing"]

    ESX.ShowNotification("BANQUE: ALARME!", "", 10000)
        ...
        if not robberyOngoing then
            ...
            for bank, values in pairs(BankHeists) do
                ...
                if distanceCheck <= 5.0 then
                    ESX.Game.Utils.DrawText3D(StartPosition, "[E] Commencer le Vol", 0.4)
                    if distanceCheck <= 1.5 then

                        if IsControlJustPressed(0, Keys["E"]) then
                            ESX.TriggerServerCallback("ddx_bankrobbery:fetchCops", function(IsEnough)
                                if IsEnough then
                                    TriggerServerEvent("ddx_bankrobbery:startRobbery", bank)
                                else
                                    ESX.ShowNotification("Il n'y a pas assez de policiers !")
                                end
                            end, MinPolice)
                        end
                    end
...
RegisterNetEvent("ddx_bankrobbery:startRobbery")
AddEventHandler("ddx_bankrobbery:startRobbery", function(bankId)
    StartRobbery(bankId)
end)

RegisterNetEvent("ddx_bankrobbery:endRobbery")
AddEventHandler("ddx_bankrobbery:endRobbery", function(bankId)
    robberyOngoing = false
end)

RegisterNetEvent("ddx_bankrobbery:changeCash")
AddEventHandler("ddx_bankrobbery:changeCash", function(bankId, newCash)
    if newCash <= 0 then
...
function StartRobbery(bankId)
    ...
    Citizen.CreateThread(function()
        while robberyOngoing do
            ...
            if distanceCheck <= 1.5 then
                ESX.Game.Utils.DrawText3D({ x = CashPosition["x"], y = CashPosition["y"], z = CashPosition["z"] }, "[E] Prendre " .. Cash, 0.4)
                if IsControlJustPressed(0, Keys["E"]) then
                    ...
                    if Cash > 0 then
                        ...
                        ESX.ShowNotification("Tout est déjà reçu !")

                        TriggerServerEvent("ddx_bankrobbery:endRobbery", bankId)
                    end
                end
            end
...
function GrabCash(bankId)
    ...
    TriggerServerEvent("ddx_bankrobbery:grabbedCash", bankId, BankHeists[bankId]["Money"], cashRecieved)
end
