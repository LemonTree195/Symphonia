...
RegisterNetEvent('ddx_vangelico_robbery:currentlyrobbing')
AddEventHandler('ddx_vangelico_robbery:currentlyrobbing', function(robb)
	holdingup = true
	store = robb
end)

RegisterNetEvent('ddx_vangelico_robbery:killblip')
AddEventHandler('ddx_vangelico_robbery:killblip', function()
    RemoveBlip(blipRobbery)
end)

RegisterNetEvent('ddx_vangelico_robbery:setblip')
AddEventHandler('ddx_vangelico_robbery:setblip', function(position)
	...
	Citizen.Wait(100)
	ESX.ShowAdvancedNotification('LSPD INFORMATIONS', 'BRAQUAGE EN COURS!', 'Une bijouterie se fait actuellement braquer, toutes les unitées son demandé sur les lieux !', 'CHAR_CALL911', 1)
	loopAlarm = true
	TriggerEvent('ddx_vangelico_robbery:loopAlarmTriggered')
end)

RegisterNetEvent('ddx_vangelico_robbery:loopAlarmTriggered')
AddEventHandler('ddx_vangelico_robbery:loopAlarmTriggered', function()
	while loopAlarm do
		PlaySoundFromCoord(-1, "scanner_alarm_os", -629.99, -236.542, 38.05, "dlc_xm_iaa_player_facility_sounds", 1, 100, 0)
		Citizen.Wait(1000)
	end
end)
...
RegisterNetEvent('ddx_vangelico_robbery:BraquageAnnule')
AddEventHandler('ddx_vangelico_robbery:BraquageAnnule', function()
	PlaySoundFrontend(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0)
	Citizen.Wait(100)
	ESX.ShowAdvancedNotification('LSPD INFORMATIONS', 'BRAQUAGE FINIT!', 'Le braqueur à été vue en train de quitté les lieux avant d\'avoir tout voler !', 'CHAR_CALL911', 1)
	loopAlarm = false
	TriggerEvent('ddx_vangelico_robbery:loopAlarmTriggered')
end)

RegisterNetEvent('ddx_vangelico_robbery:BraquageFinit')
AddEventHandler('ddx_vangelico_robbery:BraquageFinit', function()
	PlaySoundFrontend(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0)
	Citizen.Wait(500)
	ESX.ShowAdvancedNotification('LSPD INFORMATIONS', 'BRAQUAGE FINIT!', 'Le braqueur à été vue en train de quitté les lieux en ayant tout voler, il porte un sac!', 'CHAR_CALL911', 1)
	loopAlarm = false
	TriggerEvent('ddx_vangelico_robbery:loopAlarmTriggered')
end)

RegisterNetEvent('ddx_vangelico_robbery:VenteBijoux')
AddEventHandler('ddx_vangelico_robbery:VenteBijoux', function()
	PlaySoundFrontend(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0)
	Citizen.Wait(500)
	ESX.ShowAdvancedNotification('Revendeur', 'Revendeur de bijoux.', 'Hein ? T\'a des bijoux pour moi ? Parfait donne moi ça !', 'CHAR_LESTER_DEATHWISH', 1)
	loopAlarm = false
	TriggerEvent('ddx_vangelico_robbery:loopAlarmTriggered')
end)

RegisterNetEvent('ddx_vangelico_robbery:VenteBijouxRefuser')
AddEventHandler('ddx_vangelico_robbery:VenteBijouxRefuser', function()
	PlaySoundFrontend(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0)
	Citizen.Wait(500)
	ESX.ShowAdvancedNotification('Revendeur', 'Revendeur de bijoux.', 'Hein ? Tu te fou de moi t\'a rien ?! Revient quand ta au minimum 20 bijoux!', 'CHAR_LESTER_DEATHWISH', 1)
	loopAlarm = false
	TriggerEvent('ddx_vangelico_robbery:loopAlarmTriggered')
end)

RegisterNetEvent('ddx_vangelico_robbery:toofarlocal')
AddEventHandler('ddx_vangelico_robbery:toofarlocal', function(robb)
	holdingup = false
...
RegisterNetEvent('ddx_vangelico_robbery:robberycomplete')
AddEventHandler('ddx_vangelico_robbery:robberycomplete', function(robb)
	holdingup = false
	...
	while true do
				...
				if not holdingup then
					...
					if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 1.0)then
						...
						if IsPedShooting(GetPlayerPed(-1)) then
							...
							TriggerServerEvent('ddx_vangelico_robbery:rob', k)
                        end
		...
		if holdingup then
							...
							if IsControlJustReleased(1, 51) then
								...
								TriggerServerEvent('ddx_vangelico_robbery:gioielli1')
                            ...
                            hasrobbed19 = false
                            hasrobbed20 = false
						TriggerServerEvent('ddx_vangelico_robbery:endrob', store)
			...
			if (GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -622.566, -230.183, 38.057, true) > 25.5 ) then
				TriggerServerEvent('ddx_vangelico_robbery:toofar', store)
				holdingup = false
				RemoveBlip(gioielli1)
				RemoveBlip(gioielli2)
				RemoveBlip(gioielli3)
...
RegisterNetEvent('ddx_vangelico_robbery:togliblip')
AddEventHandler('ddx_vangelico_robbery:togliblip', function(robb)
	RemoveBlip(gioielli1)
	RemoveBlip(gioielli2)
	RemoveBlip(gioielli3)
	RemoveBlip(gioielli4)
...
Citizen.CreateThread(function()
		--TriggerEvent('lester:createBlip', 1269.678, -1709.697, 53.50)
    ...
    while true do
       ...
       playerPed = GetPlayerPed(-1)
		...
		if pos then
				...
				if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 608.76, -459.41, 24.74, true) <= 2 then
					ESX.ShowNotification('Tu donne t\'es bijoux au revendeur ...')
					Citizen.Wait(4000)
