        ...
        if HasScaleformMovieLoaded(scaleform) and UsingComputer then
            ...
            if GetScaleformMovieFunctionReturnBool(ClickReturn) then
                ...
                elseif Hacking and ProgramID == 86 then
                    ...
                    mHacking = false
                   -- TriggerServerEvent('ddx_phone:send', 'police', 'Un braquage est en cours à la ' .. v.BankName .. ' et le coffre de la banque a été ouvert !', {x = v.Doors[#v.Doors].Coords.x, y = v.Doors[#v.Doors].Coords.y, z = v.Doors[#v.Doors].Coords.z})
                    TriggerServerEvent('ddx_addons_gcphone:startCall', 'police', 'Un braquage est en cours à la ' .. v.BankName .. ' et le coffre de la banque a été ouvert !', {x = v.Doors[#v.Doors].Coords.x, y = v.Doors[#v.Doors].Coords.y, z = v.Doors[#v.Doors].Coords.z})
                ...
                if Hacking then
                    ...
                    if lives <= 0 then
                        ...
                        Wait(5000)
                        local v = Config.BankRobbery[hackingBank]
                        --TriggerServerEvent('ddx_phone:send', 'police', 'En anonym person har försökt att göra intrång i ' .. v.BankName .. 's system!', {x = v.Doors[#v.Doors].Coords.x, y = v.Doors[#v.Doors].Coords.y, z = v.Doors[#v.Doors].Coords.z})
                        TriggerServerEvent('ddx_addons_gcphone:startCall', 'police', 'Une personne anonyme a tenté de violer la ' .. v.BankName .. ' systèmes !', {x = v.Doors[#v.Doors].Coords.x, y = v.Doors[#v.Doors].Coords.y, z = v.Doors[#v.Doors].Coords.z})
