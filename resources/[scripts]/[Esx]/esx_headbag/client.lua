...
if closestPlayer == -1 or closestDistance > 2.0 then 
  ...
  if not HaveBagOnHead then
    TriggerServerEvent('ddx_worek:sendclosest', GetPlayerServerId(closestPlayer))
    ESX.ShowNotification('~g~Vous mettez une cagoule à ~w~' .. GetPlayerName(closestPlayer))
    TriggerServerEvent('ddx_worek:closest')
  else
...
RegisterNetEvent('ddx_worek:naloz') --This event open menu
AddEventHandler('ddx_worek:naloz', function()
    OpenBagMenu()
end)

RegisterNetEvent('ddx_worek:nalozNa') --This event put head bag on nearest player
AddEventHandler('ddx_worek:nalozNa', function(gracz)
...
RegisterNetEvent('ddx_worek:zdejmijc') --This event delete head bag from player head
AddEventHandler('ddx_worek:zdejmijc', function(gracz)
            ...
            function(data2, menu2)
              ...
              if distance ~= -1 and distance <= 2.0 then
                ...
                if data2.current.value == 'putoff' then
                  TriggerServerEvent('ddx_worek:zdejmij')
                end
              else
                ESX.ShowNotification('~r~Pas de joueur à proximité.')
              end
