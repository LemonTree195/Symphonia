...
local DiscordWebhookSystemInfos = 'https://discordapp.com/api/webhooks/692781374062461039/r5qyKqU355_ZphUNSx_g1BJR_DZQjLlw2yO-PIWWm7OlNr2ZJOfAzXPxsYtkW0RvB8Z5'
--local DiscordWebhookStart = 'https://canary.discordapp.com/api/webhooks/638061979298299914/BjgAw79lvL-hYK659BGM-AEMMYT-PaI-9Roc3dV4Fevj0FYdSYqmna4_fyw_rs62pwL3'
local DiscordWebhookKillinglogs = 'https://discordapp.com/api/webhooks/692780593880104991/9w2yJzmCnhkIGoYdQGG3GIBjr359WgtmQ9o6itCckVyUhbI4PrdlB4KVzbrat3f_dFJX'
local DiscordWebhookChat = 'https://discordapp.com/api/webhooks/692780699752595488/oSo5EsOedqcZviLlo7NCkFD--G4z2mUwZ4N9hlLdxtLwj47WhFcrgU8no51-D8-c3MHh'
local DiscordWebhookCoffreArmes = 'https://discordapp.com/api/webhooks/692781570456682547/yFSP5OkhjO7RgXfGNUQybuILeRLTWI1nCpGXw0cSfIf2t1VFtM-8vLyqmPsY4fWIGWlc'
...
if DiscordWebhookStart == 'WEBHOOK_LINK_HERE' then
	print('\n\nERROR\n' .. GetCurrentResourceName() .. ': Please add your "Start" webhook\n\n')
else
	PerformHttpRequest(DiscordWebhookStart, function(Error, Content, Head)
		if Content == '{"code": 50027, "message": "Invalid Webhook Token"}' then
			print('\n\nERROR\n' .. GetCurrentResourceName() .. ': "Start" webhook non-existing!\n\n')
		end
	end)
end
...
if DiscordWebhookCoffreArmes == 'WEBHOOK_LINK_HERE' then
	print('\n\nERROR\n' .. GetCurrentResourceName() .. ': Please add your "CoffreArmes" webhook\n\n')
else
	PerformHttpRequest(DiscordWebhookCoffreArmes, function(Error, Content, Head)
		if Content == '{"code": 50027, "message": "Invalid Webhook Token"}' then
			print('\n\nERROR\n' .. GetCurrentResourceName() .. ': "CoffreArmes" webhook non-existing!\n\n')
		end
	end)
end
...
-- System Infos
PerformHttpRequest(DiscordWebhookStart, function(Error, Content, Head) end, 'POST', json.encode({username = SystemName, content = '**@everyone Le serveur a fini de démarrer, vous pouvez désormais le rejoindre via l\'ip suivante : "35.226.137.77:30120"**'}), { ['Content-Type'] = 'application/json' })

AddEventHandler('playerConnecting', function()
	TriggerEvent('DiscordBot:ToDiscord', DiscordWebhookSystemInfos, SystemName, '```css\n' .. GetPlayerName(source) .. ' en connexion\n```', SystemAvatar, false)
end)

AddEventHandler('playerDropped', function(Reason)
	TriggerEvent('DiscordBot:ToDiscord', DiscordWebhookSystemInfos, SystemName, '```fix\n' .. GetPlayerName(source) .. ' à quitté (' .. Reason .. ')\n```', SystemAvatar, false)
end)
...
AddEventHandler('DiscordBot:ToDiscord', function(WebHook, Name, Message, Image, External, Source, TTS)
	...
	if External then
		...
		elseif WebHook:lower() == 'coffrearmes' then
			WebHook = DiscordWebhookCoffreArmes
