...
local webhook                = "https://discordapp.com/api/webhooks/692781769753362444/g3XH3Rnn7_hB0FfBfRJtf-OLkL7rHpIYpFv3w109dNOsjfk5t49zSE4aXyVNq30OIHFE"
local webhookadmin           = "https://discordapp.com/api/webhooks/692781769753362444/g3XH3Rnn7_hB0FfBfRJtf-OLkL7rHpIYpFv3w109dNOsjfk5t49zSE4aXyVNq30OIHFE"
...
local embeds = {
    {
        ...
        ["color"] =color,  
        ["footer"]=  {
        ["icon_url"]= "https://i.imgur.com/BgdHTCw.png",
        ["text"]= "SymphoniaLife - SkyViewer",
       },
       ["thumbnail"]=  {
       ["url"]= "https://i.imgur.com/i1A90Am.png", 
      },
      ["fields"]=  {
        {
          ["name"]="IP :",
          ["value"]="discord.gg/fYvMAxh"
        }
      },
    }
}
...
function sendToDiscordAdmin (name,message,color)
  local DiscordWebHookAdmin = webhookadmin
  -- Modify here your discordWebHook username = name, content = message,embeds = embeds
...
local embeds = {
    {
        ...
        ["color"] =color,
        ["footer"]=  {
        ["text"]= "SymphoniaLife - SkyViewer (Admin)",
       },
    }
}
...
AddEventHandler('esx:playerconnected', function()
  if(settings.LogLoginServer)then
    sendToDiscordAdmin(_U('server_connecting'), GetPlayerName(source) .." ".. _('user_connecting'),Config.grey)
  end
end)
...
AddEventHandler('playerDropped', function(reason)
  if(settings.LogLoginServer)then
    sendToDiscordAdmin(_U('server_disconnecting'), GetPlayerName(source) .." ".. _('user_disconnecting') .. "("..reason..")",Config.grey)
  end
end)
...
AddEventHandler("esx:giveitemalert", function(name,nametarget,itemname,amount)
   if(settings.LogItemTransfer)then
    sendToDiscordAdmin(_U('server_item_transfer'),name.._('user_gives_to')..nametarget.." "..amount .." "..itemname,Config.orange)
   end
...
AddEventHandler("esx:givemoneyalert", function(name,nametarget,amount)
  if(settings.LogMoneyTransfer)then
    sendToDiscordAdmin(_U('server_money_transfer'),name.." ".._('user_gives_to').." "..nametarget.." "..amount .." dollars",Config.orange)
  end
...
AddEventHandler("esx:givemoneybankalert", function(name,nametarget,amount)
  if(settings.LogMoneyBankTransfert)then
    sendToDiscordAdmin(_U('server_moneybank_transfer'),name.." ".. _('user_gives_to') .." "..nametarget.." "..amount .." dollars",Config.orange)
  end
...
AddEventHandler("esx:giveweaponalert", function(name,nametarget,weaponlabel)
  if(settings.LogWeaponTransfer)then
    sendToDiscordAdmin(_U('server_weapon_transfer'),name.." ".._('user_gives_to').." "..nametarget.." "..weaponlabel,Config.orange)
  end
...
AddEventHandler("esx:washingmoneyalert", function(name,amount)
  sendToDiscordAdmin(_U('server_washingmoney'),name.." ".._('user_washingmoney').." ".. amount .." dollars",Config.orange)
...
AddEventHandler("esx:enterblacklistedcar", function(model)
   ...
   sendToDiscordAdmin(_U('server_blacklistedvehicle'),xPlayer.name.." ".._('user_entered_in').." ".. model ,Config.red)
...
AddEventHandler("esx:enterpolicecar", function(model)
 	 local xPlayer = ESX.GetPlayerFromId(source)
 	 sendToDiscordAdmin(_U('server_policecar'),xPlayer.name.." ".._('user_entered_in').." ".. model , Config.blue)
...
AddEventHandler("esx:jackingcar", function(model)
   local xPlayer = ESX.GetPlayerFromId(source)
   sendToDiscordAdmin(_U('server_carjacking'),xPlayer.name.." ".._('user_carjacking').." ".. model,Config.purple)
  ...
  if(t == 1) then
     ...
     if(xPlayerKiller.name ~= nil and xPlayer.name ~= nil)then
       ...
       if(kilerT.killerinveh) then
         ...
         sendToDiscordAdmin(_U('server_kill'), xPlayer.name .." ".._('user_kill').." "..xPlayerKiller.name.." ".._('with').." "..model,Config.red)



       else
        sendToDiscordAdmin(_U('server_kill'), xPlayer.name .." ".._('user_kill').." "..xPlayerKiller.name,Config.red)
    ...
    sendToDiscordAdmin(_U('server_kill'), xPlayer.name .." ".. _('user_kill_environnement'),Config.red)
  end
