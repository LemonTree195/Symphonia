...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		if data.current.value == 'gang_wear' then
			ESX.TriggerServerCallback('ddx_skin:getPlayerSkin', function(skin, jobSkin, job2Skin)
				if skin.sex == 0 then
...
function OpenVehicleSpawnerMenu()
	...
	ESX.TriggerServerCallback('ddx_society:getVehiclesInGarage', function(vehicles)
		for i = 1, #vehicles, 1 do
			table.insert(elements, {
				label = GetDisplayNameFromVehicleModel(vehicles[i].model)..' [' .. (vehicles[i].plate or '') .. ']',
				value = vehicles[i]
			})
		end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
			...
			TriggerServerEvent('ddx_society:removeVehicleFromGarage', ActualGang.Name, vehicleProps)
		end, function(data, menu)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if distance ~= -1 and distance <= 3.0 then
					...
					if data2.current.value == 'put_in_vehicle' then
						TriggerServerEvent('ddx_gangbuild:putInVehicle', GetPlayerServerId(player))
					end

					if data2.current.value == 'out_the_vehicle' then
						TriggerServerEvent('ddx_gangbuild:OutVehicle', GetPlayerServerId(player))
					end
				else
...
function OpenBodySearchMenu(player)
	ESX.TriggerServerCallback('ddx_gangbuild:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
			...
			if data2.current.value ~= nil then
				menu2.close()
				TriggerServerEvent('ddx_gangbuild:confiscatePlayerItem', player, data2.current.itemType, data2.current.value, data2.current.amount)
...
function OpenVehicleInfosMenu(vehicleData)
	ESX.TriggerServerCallback('ddx_gangbuild:getVehicleInfos', function(infos)
		local elements = {}
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('ddx_gangbuild:getArmoryWeapons', function(weapons)
		local elements = {}

		for i = 1, #weapons, 1 do
			if weapons[i].count > 0 then
				table.insert(elements, {label = ESX.GetWeaponLabel(weapons[i].name)..' ['.. weapons[i].count ..']', rightlabel = {'[' .. weapons[i].count .. ']'}, value = weapons[i].name})
			end
		end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
			...
			ESX.TriggerServerCallback('ddx_gangbuild:removeArmoryWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon', {
		...
		ESX.TriggerServerCallback('ddx_gangbuild:addArmoryWeapon', function()
			OpenPutWeaponMenu()
		end, data.current.value)
	end, function(data, menu)
	end)
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('ddx_gangbuild:getStockItems', function(items)
		local elements = {}

		for i = 1, #items, 1 do
			table.insert(elements, {label = items[i].label.."(" .. items[i].count .. ")", rightlabel = {'(' .. items[i].count .. ')'}, value = items[i].name})
		end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('ddx_gangbuild:getStockItem', data.current.value, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('ddx_gangbuild:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('ddx_gangbuild:putStockItems', itemName, count)
						end
...
AddEventHandler('ddx_gangbuild:hasEnteredMarker', function(part)
...
AddEventHandler('ddx_gangbuild:hasExitedMarker', function(part)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)

RegisterNetEvent('ddx_gangbuild:putInVehicle')
AddEventHandler('ddx_gangbuild:putInVehicle', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed, false)
...
RegisterNetEvent('ddx_gangbuild:OutVehicle')
AddEventHandler('ddx_gangbuild:OutVehicle', function()
...
-- Display markers
Citizen.CreateThread(function()
	AddTextEntry("POINT_CLOAKROOM", "~INPUT_PICKUP~ Pour ouvrir le véstiaire")
	AddTextEntry("POINT_ARMURIE", "~INPUT_PICKUP~ Pour ouvrir l'armurie")
	AddTextEntry("POINT_VEHSPAWNER", "~INPUT_PICKUP~ Pour ouvrir le garage")
	AddTextEntry("POINT_VEHSDELETE", "~INPUT_PICKUP~ Pour ranger le véhicule")
	AddTextEntry("POINT_PATRON", "~INPUT_PICKUP~ Pour ranger les action gang")
	while true do
		Citizen.Wait(0)
		...
		if PlayerData.job2 ~= nil and ActualGang then
			...
			if GetDistanceBetweenCoords(coords, ActualGang.Cloakroom.x, ActualGang.Cloakroom.y, ActualGang.Cloakroom.z,  true) < Config.DrawDistance then
				...
				ShowFloatingHelp("POINT_CLOAKROOM", zone)
			...
			if GetDistanceBetweenCoords(coords, ActualGang.Armory.x, ActualGang.Armory.y, ActualGang.Armory.z,  true) < Config.DrawDistance then
				local zone = vector3(ActualGang.Armory.x, ActualGang.Armory.y, ActualGang.Armory.z)
				ShowFloatingHelp("POINT_ARMURIE", zone)
			...
			if GetDistanceBetweenCoords(coords, ActualGang.VehSpawner.x, ActualGang.VehSpawner.y, ActualGang.VehSpawner.z,  true) < Config.DrawDistance then
				...
				ShowFloatingHelp("POINT_VEHSPAWNER", zone)
			...
			if GetDistanceBetweenCoords(coords, ActualGang.VehDeleter.x, ActualGang.VehDeleter.y, ActualGang.VehDeleter.z,  true) < Config.DrawDistance then
				...
				ShowFloatingHelp("POINT_VEHSDELETE", zone)
			...
			if PlayerData.job2 ~= nil and PlayerData.job2.grade_name == 'boss' then
				if GetDistanceBetweenCoords(coords, ActualGang.BossActions.x, ActualGang.BossActions.y, ActualGang.BossActions.z, true) < Config.DrawDistance then
					...
					ShowFloatingHelp("POINT_PATRON", zone)
		...
		if PlayerData.job2 ~= nil and ActualGang then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastPart ~= currentPart)) then
				if (LastPart ~= nil) and (LastPart ~= currentPart) then
					TriggerEvent('ddx_gangbuild:hasExitedMarker', LastPart)
					hasExited = true
				end
				...
				TriggerEvent('ddx_gangbuild:hasEnteredMarker', currentPart)
			end
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0, 38) and PlayerData.job2 ~= nil and ActualGang and (GetGameTimer() - GUI.Time) > 150 then
				...
				if CurrentAction == 'delete_vehicle' then
					...
					TriggerServerEvent('ddx_society:putVehicleInGarage', ActualGang.Name, vehicleProps)
				...
				if CurrentAction == 'menu_boss_actions' then
					...
					TriggerEvent('ddx_society:openBossMenu', ActualGang.Name, function(data, menu)
						CurrentAction = 'menu_boss_actions'
						CurrentActionMsg = _U('open_bossmenu')
						CurrentActionData = {}
					end, {wash = false})
