...
RegisterNetEvent('ddx_garbagejob:setbin')
AddEventHandler('ddx_garbagejob:setbin', function(binpos, platenumber,  bags)
	if isInService then
...
RegisterNetEvent('ddx_garbagejob:addbags')
AddEventHandler('ddx_garbagejob:addbags', function(platenumber, bags, crewmember)
	if isInService then
...
RegisterNetEvent('ddx_garbagejob:startpayrequest')
AddEventHandler('ddx_garbagejob:startpayrequest', function(platenumber, amount)
	if isInService then
		if platenumb == platenumber then
			TriggerServerEvent('ddx_garbagejob:pay', amount)
			platenumb = nil
		end
	end
end)

RegisterNetEvent('ddx_garbagejob:removedbag')
AddEventHandler('ddx_garbagejob:removedbag', function(platenumber)
	if isInService then
		if platenumb == platenumber then
...
RegisterNetEvent('ddx_garbagejob:countbagtotal')
AddEventHandler('ddx_garbagejob:countbagtotal', function(platenumber)
	if isInService then
		if platenumb == platenumber then
			if not iscurrentboss then
			TriggerServerEvent('ddx_garbagejob:bagsdone', platenumb, totalbagpay)
			totalbagpay = 0
			end
		end
	end
...
RegisterNetEvent('ddx_garbagejob:clearjob')
AddEventHandler('ddx_garbagejob:clearjob', function(platenumber)
	if platenumb == platenumber then
...
AddEventHandler('ddx_garbagejob:hasEnteredMarker', function(zone)
...
AddEventHandler('ddx_garbagejob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()    
    CurrentAction = nil
	CurrentActionMsg = ''
end) 
...
function nouvelledestination()
	...
	TriggerServerEvent('ddx_garbagejob:requestpay', platenumb,  temppayamount)
...
function SelectBinandCrew()
	...
	TriggerServerEvent("ddx_garbagejob:binselect", trashcollectionpos, platenumb, bagsoftrash)
end
				...
				if CollectionAction == 'collection' then
					...
					if dist <= 3.5 then
						if currentbag > 0 then
							...
							TriggerServerEvent('ddx_garbagejob:bagremoval', platenumb)
							trashcollection = false
							Citizen.Wait(4000)
							...
							if iscurrentboss then
								...
								TriggerServerEvent('ddx_garbagejob:reportbags', platenumb)
								Citizen.Wait(4000)
								ClearPedTasks(PlayerPedId())
        	        	        setring = false
            	        	    bagsoftrash = math.random(2,10)
								...
								TriggerServerEvent('ddx_garbagejob:endcollection', platenumb)
		...
		if not paused then 
			...
			if IsJobgarbage() then
				...
				if isInMarker and not hasAlreadyEnteredMarker then
					...
					TriggerEvent('ddx_garbagejob:hasEnteredMarker', currentZone)
				end

				if not isInMarker and hasAlreadyEnteredMarker then
					hasAlreadyEnteredMarker = false
					TriggerEvent('ddx_garbagejob:hasExitedMarker', lastZone)
				end
			end
		end
