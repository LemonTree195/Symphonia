...
AddEventHandler('playerSpawned', function()
	...
	if FirstSpawn then
		...
		ESX.TriggerServerCallback('ddx_ambulancejob:getDeathStatus', function(isDead)
...
function OnPlayerDeath()
	...
	TriggerServerEvent('ddx_ambulancejob:setDeathStatus', true)
...
RegisterNetEvent('ddx_ambulancejob:useItem')
AddEventHandler('ddx_ambulancejob:useItem', function(itemName)
	ESX.UI.Menu.CloseAll()
	...
	if itemName == 'medikit' then
		...
		ESX.Streaming.RequestAnimDict(lib, function()
			...
			TriggerEvent('ddx_ambulancejob:heal', 'big', true)
	...
	elseif itemName == 'bandage' then
		...
		ESX.Streaming.RequestAnimDict(lib, function()
			...
			TriggerEvent('ddx_ambulancejob:heal', 'small', true)
...
--	TriggerServerEvent('ddx_phone:send', 'ambulance', _U('distress_message'), false, {
--		x = coords.x,
--		y = coords.y,
--		z = coords.z
--	})
...
RegisterNetEvent('ddx_ambulancejob:notif')
AddEventHandler('ddx_ambulancejob:notif', function()
...
function StartDeathTimer()
	...
	if Config.EarlyRespawnFine then
		ESX.TriggerServerCallback('ddx_ambulancejob:checkBalance', function(canPay)
			canPayFine = canPay
		end)
	end
	...
	Citizen.CreateThread(function()
		...
		while bleedoutTimer > 0 and IsDead do
			...
			elseif Config.EarlyRespawnFine and canPayFine then
				...
				if IsControlPressed(0, Keys['E']) and timeHeld > 60 then
					TriggerServerEvent('ddx_ambulancejob:payFine')
					RemoveItemsAfterRPDeath()
					break
				end
			end
...
function RemoveItemsAfterRPDeath()
	...
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed)
	TriggerServerEvent('ddx_ambulancejob:setDeathStatus', false)
	...
	Citizen.CreateThread(function()
		...
		local name = GetPlayerName(PlayerId())
		local x, y, z = table.unpack(GetEntityCoords(ped, true))
		TriggerServerEvent('ddx_ambulance:NotificationBlipsX', x, y, z, name)
...
RegisterNetEvent('ddx_ambulancejob:NotificationBlipsX2')
AddEventHandler('ddx_ambulancejob:NotificationBlipsX2', function(blipId, x, y, z)
	Nombreinter = Nombreinter - 1
	if Nombreinter < 0 then
		Nombreinter = 0
	end
...
RegisterNetEvent('ddx_ambulancejob:revive')
AddEventHandler('ddx_ambulancejob:revive', function()
	Nombreinter = Nombreinter - 1
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed)
...
RegisterNetEvent('ddx_ambulancejob:revive2')
AddEventHandler('ddx_ambulancejob:revive2', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed)
	...
	Citizen.CreateThread(function()
		...
		ESX.ShowAdvancedNotification('STAFF INFO', 'STAFF ~g~REVIVE', 'Tu à été revive par un staff.', 'CHAR_DEVIN', 8)
	end)
end)
