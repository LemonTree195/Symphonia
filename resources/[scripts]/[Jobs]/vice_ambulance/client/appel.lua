...
AddEventHandler("AppelemsTropBien", function(coords, id)
	...
	AppelCoords = coords
	AppelID = id
	ESX.ShowAdvancedNotification("ems", "~b~Demande de ems", "Quelqu'un à besoin d'un ems !\n~g~Y~w~ pour prendre l'appel\n~r~X~w~ pour refuser", "CHAR_CALL911", 8)
end)
...
Citizen.CreateThread(function()
     while true do
		...
		if IsControlJustPressed(1, 246) and AppelDejaPris == true then
			ESX.ShowAdvancedNotification("ems", "~b~Demande de ems", "L'appel à déja été pris, désolé.", "CHAR_CALL911", 8)
		end
     end
end)
...
AddEventHandler("emsAppelPris", function(Xid, XAppelCoords)
	ESX.ShowAdvancedNotification("ems", "~b~Demande de ems", "Vous avez pris l'appel, suivez la route GPS.", "CHAR_CALL911", 8)   
     ...
     while rea do
          if GetDistanceBetweenCoords(XAcoords, GetEntityCoords(GetPlayerPed(-1))) < 10.0 then
               ...
               if IsControlJustReleased(0, 38) then
				...
				TriggerServerEvent('ddx_ambulancejob:revive', XAid)
