...
AddEventHandler("ddx_miner:washing", function()
    Washing()
end)

RegisterNetEvent("ddx_miner:remelting")
AddEventHandler("ddx_miner:remelting", function()
    Remelting()
end)

RegisterNetEvent('ddx_miner:timer')
AddEventHandler('ddx_miner:timer', function()
    local timer = 0
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.WashingX, Config.WashingY, Config.WashingZ, true) < 5 then
                Draw3DText( Config.WashingX, Config.WashingY, Config.WashingZ+0.5 -1.400, ('Lavement des pierres en progression ' .. timer .. '%'), 4, 0.1, 0.1)
            end
            if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.RemeltingX, Config.RemeltingY, Config.RemeltingZ, true) < 5 then
                Draw3DText( Config.RemeltingX, Config.RemeltingY, Config.RemeltingZ+0.5 -1.400, ('Refonte des pierres en cours ' .. timer .. '%'), 4, 0.1, 0.1)
            end
            if timer == 100 then
                timer = 0
                break
...
RegisterNetEvent('ddx_miner:createblips')
AddEventHandler('ddx_miner:createblips', function()
    Citizen.CreateThread(function()
        while true do 
            Citizen.Wait(1)
                if blips == true and blipActive == false then
                    ...
                    BeginTextCommandSetBlipName("STRING")
                    AddTextComponentString("Lavement de pierre")
                    EndTextCommandSetBlipName(blip2)   
                    SetBlipSprite(blip3, 365)
                    SetBlipColour(blip3, 5)
                    SetBlipAsShortRange(blip3, true)
                    BeginTextCommandSetBlipName("STRING")
                    AddTextComponentString("Fonte de pierre")
...
Citizen.CreateThread(function()
    ...
    SetBlipAsShortRange(blip1, true)
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString("Miner's cloakroom")
    EndTextCommandSetBlipName(blip1)   
end)
...
Citizen.CreateThread(function()
	...
	local ped = PlayerPedId()
        Citizen.Wait(1)
            if PlayerData.job ~= nil and PlayerData.job.name and not IsEntityDead( ped ) then
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(ped), locations[i].x, locations[i].y, locations[i].z, true) < 25 and mineActive == false then
                DrawMarker(20, locations[i].x, locations[i].y, locations[i].z, 0, 0, 0, 0, 0, 100.0, 1.0, 1.0, 1.0, 0, 155, 253, 155, 0, 0, 2, 0, 0, 0, 0)
                    if GetDistanceBetweenCoords(GetEntityCoords(ped), locations[i].x, locations[i].y, locations[i].z, true) < 1 then
                        ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ aller à l'exploitation minière.")
                            if IsControlJustReleased(1, 51) then
                                Animation()
                                mineActive = true
                            end
        ...
        if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.WashingX, Config.WashingY, Config.WashingZ, true) < 25 and washingActive == false then
            DrawMarker(20, Config.WashingX, Config.WashingY, Config.WashingZ, 0, 0, 0, 0, 0, 55.0, 1.0, 1.0, 1.0, 0, 155, 253, 155, 0, 0, 2, 0, 0, 0, 0)
                if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.WashingX, Config.WashingY, Config.WashingZ, true) < 1 then
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour laver les pierres.")
                        if IsControlJustReleased(1, 51) then
                            TriggerServerEvent("ddx_miner:washing")
                end
            end
        end
    end
        ...
        if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.RemeltingX, Config.RemeltingY, Config.RemeltingZ, true) < 25 and remeltingActive == false then
            DrawMarker(20, Config.RemeltingX, Config.RemeltingY, Config.RemeltingZ, 0, 0, 0, 0, 0, 55.0, 1.0, 1.0, 1.0, 0, 155, 253, 155, 0, 0, 2, 0, 0, 0, 0)
                if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.RemeltingX, Config.RemeltingY, Config.RemeltingZ, true) < 1 then
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour faire fondre les pierres.")
                        if IsControlJustReleased(1, 51) then
                          TriggerServerEvent("ddx_miner:remelting")  
                    end
                end
            end
        end
...
function Cloakroom()
    local elements = {
        {label = 'Vetements civil',   value = 'cloakroom1'},
        {label = 'Vetements de travail',      value = 'cloakroom2'},
        {label = 'Voiture',       value = 'vehicle'},
    }
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'miner_actions', {
        ...
        if data.current.value == 'cloakroom1' then
            ...
            blips = false
            blipActive = false
            TriggerEvent("ddx_miner:createblips")
        ...
        elseif data.current.value == 'vehicle' then
            ...
            ESX.ShowNotification("Le véhicule a été sorti du garage.")
        end
    end)
end
...
function Jeweler()
    local elements = {
        {label = 'Vente de diaments',   value = 'diamonds'},
        {label = 'Vente d\'or',      value = 'gold'},
        {label = 'Vente de fer',       value = 'iron'},
        {label = 'Vente de pierre',       value = 'copper'},
    }
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'jeweler_actions', {
        ...
        if data.current.value == 'diamonds' then
            menu.close()
            TriggerServerEvent("ddx_miner:selldiamond")
        elseif data.current.value == 'gold' then
            menu.close()
            TriggerServerEvent("ddx_miner:sellgold")
        elseif data.current.value == 'iron' then
            menu.close()
            TriggerServerEvent("ddx_miner:selliron")
        elseif data.current.value == 'copper' then
            menu.close()
            TriggerServerEvent("ddx_miner:sellcopper")
        end
    end)
end
    ...
    Citizen.CreateThread(function()
		...
		local ped = PlayerPedId()	
                ...
                if impacts == 5 then
                    ...
                    mineActive = false
                    impacts = 0
                    TriggerServerEvent("ddx_miner:givestone")
                    break
                end        
        end
    end)
