			...
			if Config.MaxInService ~= -1 then
				...
				ESX.TriggerServerCallback('ddx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('ddx_service:notifyAllInService', notification, 'vigne')

						TriggerServerEvent('ddx_service:disableService', 'vigne')
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('ddx_society:openBossMenu', 'vigne', function(data, menu)
					menu.close()
				end)
			end
		...
		ESX.TriggerServerCallback('ddx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					TriggerServerEvent('ddx_society:removeVehicleFromGarage', 'vigne', vehicleProps)
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('ddx_billing:sendBill24', GetPlayerServerId(closestPlayer), 'society_vigne', 'Vigneron', amount)
								end)
							end
						end
					end,
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('ddx_vigneronjob:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('ddx_vigneronjob:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('ddx_vigneronjob:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('ddx_vigneronjob:putStockItems', itemName, count)
						end
...
AddEventHandler('ddx_vigneronjob:hasEnteredMarker', function(zone)
	...
	if zone == 'SellFarm2' and PlayerData.job ~= nil and PlayerData.job.name == 'vigne'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end
...
AddEventHandler('ddx_vigneronjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'RaisinFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' then
		TriggerServerEvent('ddx_vigneronjob:stopHarvest')
	end  
	if (zone == 'TraitementVin' or zone == 'TraitementJus') and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' then
		TriggerServerEvent('ddx_vigneronjob:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' then
		TriggerServerEvent('ddx_vigneronjob:stopSell')
	end
	if (zone == 'SellFarm2') and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' then
		TriggerServerEvent('ddx_vigneronjob:stopSell')
	end
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'vigne' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('ddx_vigneronjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('ddx_vigneronjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['X']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' and (GetGameTimer() - GUI.Time) > 150 then
				local playerPed = GetPlayerPed(-1)
				ClearPedTasks(playerPed)
				TriggerServerEvent('ddx_vigneronjob:stopHarvest')
				TriggerServerEvent('ddx_vigneronjob:stopTransform')
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'raisin_harvest' then
					...
					Citizen.CreateThread(function()
						...
						TriggerServerEvent('ddx_vigneronjob:startHarvest', CurrentActionData.zone)
					end)
				end
				if CurrentAction == 'jus_traitement' then
					...
					Citizen.CreateThread(function()
						FreezeEntityPosition(playerPed,  true)
						ESX.ShowHelpNotification('Appuyez sur ~r~X~w~ pour arreter la transformation.', false , true , 100)	
						TriggerServerEvent('ddx_vigneronjob:startTransform', CurrentActionData.zone)
					end)
				end
				...
				if CurrentAction == 'farm_resell' then
					...
					Citizen.CreateThread(function()
						FreezeEntityPosition(playerPed,  true)
						ESX.ShowHelpNotification('Appuyez sur ~r~X~w~ pour arreter la transformation.', false , true , 100)	
						TriggerServerEvent('ddx_vigneronjob:startSell', CurrentActionData.zone)
					end)
				end
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('ddx_society:putVehicleInGarage', 'vigne', vehicleProps)
					end
