...
AddEventHandler('esx:playerDropped', function(source)
  ...
  TriggerEvent('ddx_addonaccount:getAccount', 'caution', xPlayer.identifier, function(account)
    account.addMoney(caution)
  end)
end)

RegisterServerEvent('ddx_jobs:setCautionInCaseOfDrop')
AddEventHandler('ddx_jobs:setCautionInCaseOfDrop', function(caution)
  local _source = source
  local xPlayer = ESX.GetPlayerFromId(_source)
  xPlayer.set('caution', caution)
end)

RegisterServerEvent('ddx_jobs:giveBackCautionInCaseOfDrop')
AddEventHandler('ddx_jobs:giveBackCautionInCaseOfDrop', function()
  local _source = source
  local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('ddx_jobs:startWork')
AddEventHandler('ddx_jobs:startWork', function(item)
  local source = source
  PlayersWorking[source] = true
  Work(source, item)
end)

RegisterServerEvent('ddx_jobs:stopWork')
AddEventHandler('ddx_jobs:stopWork', function()
  local source = source
  PlayersWorking[source] = false
end)

RegisterServerEvent('ddx_jobs:caution1')
AddEventHandler('ddx_jobs:caution1', function(cautionType, cautionAmount, spawnPoint, vehicle)
  local source = source
  local xPlayer = ESX.GetPlayerFromId(source)
  ...
  if cautionType == "take" then
    ...
    TriggerClientEvent('ddx_jobs:spawnJobVehicle', source, spawnPoint, vehicle)
