...
AddEventHandler('esx:playerLoaded', function(xPlayer)
    PlayerData = xPlayer
    TriggerServerEvent('ddx_jobs:giveBackCautionInCaseOfDrop')
    refreshBlips()
end)
...
AddEventHandler('ddx_jobs:action', function(job, zone)
  menuIsShowed = true
  ...
  elseif zone.Type == "work" then
    ...
    if IsPedInAnyVehicle(playerPed, 0) then
      ...
      TriggerServerEvent('ddx_jobs:startWork', zone.Item)
    end
  elseif zone.Type == "vehspawner" then
    TriggerServerEvent('ddx_jobs:requestPlayerData', 'refresh_bank_account')
    local spawnpt = nil
    ...
    TriggerServerEvent('ddx_jobs:setCautionInCaseOfDrop', caution)
            ...
            if IsPedInAnyVehicle(playerPed, 0) then
              ...
              for i=1, #myPlate, 1 do
                if (myPlate[i] == plate) and (playerPed == driverPed) then
                  TriggerServerEvent('ddx_jobs:caution1', "give_back", cautionVehicleInCaseofDrop, 0, 0)
                  ...
                  if vehicleObjInCaseofDrop.HasCaution then
                    ...
                    TriggerServerEvent('ddx_jobs:setCautionInCaseOfDrop', 0)
                  end
...
AddEventHandler('ddx_jobs:hasExitedMarker', function(zone)
  TriggerServerEvent('ddx_jobs:stopWork')
  hintToDisplay = "no hint to display"
  menuIsShowed = false
  hintIsShowed = false
  isInMarker = false
...
function spawncar(spawnPoint, vehicle)
  hintToDisplay = "no hint to display"
  hintIsShowed = false
  TriggerServerEvent('ddx_jobs:caution1', "take", cautionVehicleInCaseofDrop, spawnPoint, vehicle)
end

RegisterNetEvent('ddx_jobs:spawnJobVehicle')
AddEventHandler('ddx_jobs:spawnJobVehicle', function(spawnPoint, vehicle)
  local playerPed = GetPlayerPed(-1)
      ...
      if zones ~= nil then
        ...
        if IsControlJustReleased(0, Keys["E"]) and not menuIsShowed and isInMarker then
          if onDuty or zone.Type == "cloakroom" or PlayerData.job.name == "reporter" then
            TriggerEvent('ddx_jobs:action', job, zone)
          end
        end
        ...
        if not isInMarker and hasAlreadyEnteredMarker then
          ...
          TriggerEvent('ddx_jobs:hasExitedMarker', zone)
        end
      end
    end
  end
		...
		if vehicleInCaseofDrop ~= nil then
			if onDuty and IsVehicleModel(vehicleInCaseofDrop, vehicleHashInCaseofDrop) then
				...
				if vehicleOldHealthInCaseofDrop ~= vehicleHealth then
					...
					TriggerServerEvent('ddx_jobs:setCautionInCaseOfDrop', cautionValue)
				end
			end
		else
			Citizen.Wait(1000)
