...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
			...
			if Config.MaxInService ~= -1 then
				...
				ESX.TriggerServerCallback('ddx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('ddx_service:notifyAllInService', notification, 'tabac')

						TriggerServerEvent('ddx_service:disableService', 'tabac')
		...
		if Config.MaxInService ~= -1 and data.current.value ~= 'citizen_wear' then
			...
			if data.current.value == 'tabac_wear' then
				ESX.TriggerServerCallback('ddx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('ddx_society:openBossMenu', 'tabac', function(data, menu)
					menu.close()
				end)
			end
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('ddx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					TriggerServerEvent('ddx_society:removeVehicleFromGarage', 'tabac', vehicleProps)
		...
		local elements = {
			{label = 'Véhicule de Travail',  value = 'sadler'},
		}
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('ddx_billing:sendBill24', GetPlayerServerId(closestPlayer), 'society_tabac', 'Tabac', amount)
								end)
							end
						end
					end,
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('ddx_tabac:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('ddx_tabac:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('ddx_tabac:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('ddx_tabac:putStockItems', itemName, count)
						end
...
AddEventHandler('ddx_tabac:hasEnteredMarker', function(zone)
	...
	if zone == 'TraitementTabac' and PlayerData.job ~= nil and PlayerData.job.name == 'tabac'  then
		CurrentAction     = 'tabac_traitement'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementTabacBlond' and PlayerData.job ~= nil and PlayerData.job.name == 'tabac'  then
		CurrentAction     = 'tabacb_traitement'
	...
	if zone == 'SellFarm2' and PlayerData.job ~= nil and PlayerData.job.name == 'tabac'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'TabacActions' and PlayerData.job ~= nil and PlayerData.job.name == 'tabac' then
		CurrentAction     = 'tabac_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
...
AddEventHandler('ddx_tabac:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'TabacFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'tabac' then
		TriggerServerEvent('ddx_tabac:stopHarvest')
	end  
	if (zone == 'TraitementTabac' or zone == 'TraitementTabacBlond') and PlayerData.job ~= nil and PlayerData.job.name == 'tabac' then
		TriggerServerEvent('ddx_tabac:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'tabac' then
		TriggerServerEvent('ddx_tabac:stopSell')
	end
	if (zone == 'SellFarm2') and PlayerData.job ~= nil and PlayerData.job.name == 'tabac' then
		TriggerServerEvent('ddx_tabac:stopSell')
	end
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'tabac' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('ddx_tabac:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('ddx_tabac:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'tabac' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'tabac_harvest' then
					...
					Citizen.CreateThread(function()
						...
						ESX.ShowHelpNotification('Appuyez sur ~r~X~w~ pour arreter la recolte.', false , true , 100)
						TriggerServerEvent('ddx_tabac:startHarvest', CurrentActionData.zone)
						
					end)
				end
				if CurrentAction == 'tabacb_traitement' then
					local playerPed = GetPlayerPed(-1)
					Citizen.CreateThread(function()
						FreezeEntityPosition(playerPed,  true)
						ESX.ShowHelpNotification('Appuyez sur ~r~X~w~ pour arreter la transformation.', false , true , 100)						TriggerServerEvent('ddx_tabac:startTransform', CurrentActionData.zone)
					end)
				end
				if CurrentAction == 'tabac_traitement' then
					local playerPed = GetPlayerPed(-1)
					Citizen.CreateThread(function()
						FreezeEntityPosition(playerPed,  true)
						ESX.ShowHelpNotification('Appuyez sur ~r~X~w~ pour arreter la transformation.', false , true , 100)						TriggerServerEvent('ddx_tabac:startTransform', CurrentActionData.zone)
				...
				if CurrentAction == 'farm_resell' then
					...
					Citizen.CreateThread(function()
						FreezeEntityPosition(playerPed,  true)
						ESX.ShowHelpNotification('Appuyez sur ~r~X~w~ pour arreter la vente.', false , true , 100)						TriggerServerEvent('ddx_tabac:startSell', CurrentActionData.zone)
					end)
				end
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('ddx_society:putVehicleInGarage', 'tabac', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['X']) and PlayerData.job ~= nil and PlayerData.job.name == 'tabac' and (GetGameTimer() - GUI.Time) > 150 then
			local playerPed = GetPlayerPed(-1)
			Citizen.CreateThread(function()
