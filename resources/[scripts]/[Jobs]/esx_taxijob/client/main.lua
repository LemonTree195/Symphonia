	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('ddx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner',
			{
				...
				TriggerServerEvent('ddx_society:removeVehicleFromGarage', 'taxi', vehicleProps)
...
function DeleteJobVehicle()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		TriggerServerEvent('ddx_society:putVehicleInGarage', 'taxi', vehicleProps)
		...
		if IsInAuthorizedVehicle() then
			...
			if Config.MaxInService ~= -1 then
				TriggerServerEvent('ddx_service:disableService', 'taxi')
			end
		else
...
function OpenTaxiActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'taxi_actions',
	{
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('ddx_society:openBossMenu', 'taxi', function(data, menu)
				menu.close()
			end, {wash = false})
		end
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						TriggerServerEvent('ddx_billing:sendBill24', GetPlayerServerId(closestPlayer), 'society_taxi', 'Taxi', amount)
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('ddx_taxijob:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('ddx_taxijob:getStockItem', itemName, count)
					Citizen.Wait(1000)
					OpenGetStocksMenu()
				end
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('ddx_taxijob:getPlayerInventory', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					-- todo: refresh on callback
					TriggerServerEvent('ddx_taxijob:putStockItems', itemName, count)
					Citizen.Wait(1000)
					OpenPutStocksMenu()
				end
			end, function(data2, menu2)
...
AddEventHandler('ddx_taxijob:hasEnteredMarker', function(zone)
...
AddEventHandler('ddx_taxijob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)
	...
	while true do
		...
		if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'taxi' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('ddx_taxijob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('ddx_taxijob:hasExitedMarker', LastZone)
			end
		else
			Citizen.Wait(500)
		end
					...
					if IsPedSittingInVehicle(CurrentCustomer, vehicle) then
						if CustomerEnteredVehicle then
							...
							if targetDistance <= 10.0 then
								...
								TaskGoStraightToCoord(CurrentCustomer, TargetCoords.x, TargetCoords.y, TargetCoords.z, 1.0, -1, 0.0, 0.0)
								SetEntityAsMissionEntity(CurrentCustomer, false, true)
								TriggerServerEvent('ddx_taxijob:fini')
								RemoveBlip(DestinationBlip)
