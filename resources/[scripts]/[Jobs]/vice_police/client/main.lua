...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{ label = 'Tenue Manche Courte', value = 'tenu_lspd' },
		...
		{ label = 'Tenue DOA Gillet', value = 'tenu_doag' },
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		if data.current.value == 'citizen_wear' then
			if Config.EnableNonFreemodePeds then
				ESX.TriggerServerCallback('ddx_skin:getPlayerSkin', function(skin, jobSkin)
			...
			if Config.MaxInService ~= -1 then
				ESX.TriggerServerCallback('ddx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('ddx_service:notifyAllInService', notification, 'police')

						TriggerServerEvent('ddx_service:disableService', 'police')
						TriggerEvent('ddx_policejob:updateBlip')
...
function OpenVehicleSpawnerMenu(type, station, part, partNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		elseif data.current.action == 'garage' then
			...
			ESX.TriggerServerCallback('ddx_vehicleshop:retrieveJobVehicles', function(jobVehicles)
						...
						if data2.current.stored then
							...
							if foundSpawn then
								...
								ESX.Game.SpawnVehicle(data2.current.model, spawnPoint.coords, spawnPoint.heading, function(vehicle)
									...
									TriggerServerEvent('ddx_vehicleshop:setJobVehicleState', data2.current.vehicleProps.plate, false)
...
function StoreNearbyVehicle(playerCoords)
	...
	ESX.TriggerServerCallback('ddx_policejob:storeNearbyVehicle', function(storeSuccess, foundNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('ddx_policejob:buyJobVehicle', function (bought)
					if bought then
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'body_search' then
						TriggerServerEvent('ddx_policejob:message', GetPlayerServerId(closestPlayer), _U('being_searched'))
						OpenBodySearchMenu(closestPlayer)
					elseif action == 'handcuff' then
						--TriggerServerEvent('ddx_policejob:handcuff', GetPlayerServerId(closestPlayer))
						...
						if distance <= 2.0 then
							TriggerServerEvent('ddx_policejob:requestarrest', target_id, playerheading, playerCoords, playerlocation)
							Wait(5000)
							TriggerServerEvent('ddx_policejob:handcuff', GetPlayerServerId(closestPlayer))
						else
					...
					elseif action == 'handcuff2' then
						...
						playerCoords = GetEntityCoords(GetPlayerPed(-1))
						local target_id = GetPlayerServerId(target)
						TriggerServerEvent('ddx_policejob:requestrelease', target_id, playerheading, playerCoords, playerlocation)
						Wait(5000)
						TriggerServerEvent('ddx_policejob:handcuff', GetPlayerServerId(closestPlayer))
					elseif action == 'drag' then
						TriggerServerEvent('ddx_policejob:drag', GetPlayerServerId(closestPlayer))
					elseif action == 'put_in_vehicle' then
						TriggerServerEvent('ddx_policejob:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'out_the_vehicle' then
						TriggerServerEvent('ddx_policejob:OutVehicle', GetPlayerServerId(closestPlayer))
					...
					if amount == nil or amount <= 0 then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							Citizen.CreateThread(function()
								...
								TriggerServerEvent('ddx_billing:sendBill24', GetPlayerServerId(closestPlayer), 'society_police', 'Police', amount)
							end)
						end
					end
				end,
		...
		elseif data.current.value == 'lspd_equipement' then
			...
			elseif grade == 'intendent' then
				table.insert(elements, {label = ('Equipement sergeant-chef'), value = 'equipement_sergeant'})
			elseif grade == 'lieutenant' then
				table.insert(elements, {label = ('Equipement Lieutenant'), value = 'equipement_lieutenant'})
			elseif grade == 'chef' then
				table.insert(elements, {label = ('Equipement Capitaine'), value = 'equipement_lieutenant'})
			elseif grade == 'boss' then
...
function OpenIdentityCardMenu(player)
	ESX.TriggerServerCallback('ddx_policejob:getOtherPlayerData', function(data)
		local elements = {}
	...
	ESX.TriggerServerCallback('ddx_policejob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
			...
			if data.current.value then
				TriggerServerEvent('ddx_policejob:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
...
function OpenFineCategoryMenu(player, category)
	ESX.TriggerServerCallback('ddx_policejob:getFineList', function(fines)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('ddx_billing:sendBill24', GetPlayerServerId(player), 'society_police', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
				TriggerServerEvent('ddx_billing:sendBill24', GetPlayerServerId(player), '', _U('fine_total', data.current.fineLabel), data.current.amount)
			end
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('ddx_policejob:getVehicleFromPlate', function(owner, found)
				if found then
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('ddx_policejob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license',
		{
			...
			TriggerServerEvent('ddx_policejob:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))

			TriggerServerEvent('ddx_license:removeLicense', GetPlayerServerId(player), data.current.type)
...
function OpenUnpaidBillsMenu(player)
	...
	ESX.TriggerServerCallback('ddx_billing:getTargetBills', function(bills)
...
function OpenVehicleInfosMenu(vehicleData)
	ESX.TriggerServerCallback('ddx_policejob:getVehicleInfos', function(retrivedInfo)
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('ddx_policejob:getArmoryWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
			...
			ESX.TriggerServerCallback('ddx_policejob:removeArmoryWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon', {
		...
		ESX.TriggerServerCallback('ddx_policejob:addArmoryWeapon', function()
			OpenPutWeaponMenu()
...
--			end
--		else
--			ESX.TriggerServerCallback('ddx_policejob:buyWeapon', function(bought)
--				if bought then
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('ddx_policejob:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('ddx_policejob:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('ddx_policejob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('ddx_policejob:putStockItems', itemName, count)
...
AddEventHandler('esx:setJob', function(job)
	...
	isPlayerWhitelisted = refreshPlayerWhitelisted()
	Citizen.Wait(5000)
	TriggerServerEvent('ddx_policejob:forceBlip')
end)
...
AddEventHandler('ddx_policejob:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('ddx_policejob:hasExitedMarker', function(station, part, partNum)
...
AddEventHandler('ddx_policejob:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()
...
AddEventHandler('ddx_policejob:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('ddx_policejob:handcuff')
AddEventHandler('ddx_policejob:handcuff', function()
...
RegisterNetEvent('ddx_policejob:unrestrain')
AddEventHandler('ddx_policejob:unrestrain', function()
	if IsHandcuffed then
		local playerPed = PlayerPedId()
		IsHandcuffed = false
...
RegisterNetEvent('ddx_policejob:drag')
AddEventHandler('ddx_policejob:drag', function(copId)
...
RegisterNetEvent('ddx_policejob:putInVehicle')
AddEventHandler('ddx_policejob:putInVehicle', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed)
...
RegisterNetEvent('ddx_policejob:OutVehicle')
AddEventHandler('ddx_policejob:OutVehicle', function()
	local playerPed = PlayerPedId()
	...
	while true do
		...
		if PlayerData.job and PlayerData.job.name == 'police' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('ddx_policejob:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('ddx_policejob:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('ddx_policejob:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity then
				TriggerEvent('ddx_policejob:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'police' then
				...
				elseif CurrentAction == 'menu_boss_actions' then
					ESX.UI.Menu.CloseAll()
					TriggerEvent('ddx_society:openBossMenu', 'police', function(data, menu)
						menu.close()
...
--RegisterNetEvent('ddx_renfort:blip')
--AddEventHandler('ddx_renfort:blip', function(coords)
--	print(coords)
--	--local ped = GetPlayerPed(id)
--	local alpha = 2500
--	--local blip = GetBlipFromEntity(ped)
...
RegisterNetEvent('ddx_policejob:updateBlip')
AddEventHandler('ddx_policejob:updateBlip', function()
	...
	if PlayerData.job and PlayerData.job.name == 'police' then
		ESX.TriggerServerCallback('ddx_society:getOnlinePlayers', function(players)
...
AddEventHandler('playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('ddx_policejob:unrestrain')

	if not hasAlreadyJoined then
		TriggerServerEvent('ddx_policejob:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('ddx_policejob:unrestrain')
		TriggerEvent('ddx_phone:removeSpecialContact', 'police')
...
RegisterNetEvent('ddx_policejob:getarrested')
AddEventHandler('ddx_policejob:getarrested', function(playerheading, playercoords, playerlocation)
...
RegisterNetEvent('ddx_policejob:doarrested')
AddEventHandler('ddx_policejob:doarrested', function()
	Citizen.Wait(250)
	loadanimdict('mp_arrest_paired')
...
RegisterNetEvent('ddx_policejob:douncuffing')
AddEventHandler('ddx_policejob:douncuffing', function()
	Citizen.Wait(250)
	loadanimdict('mp_arresting')
...
RegisterNetEvent('ddx_policejob:getuncuffed')
AddEventHandler('ddx_policejob:getuncuffed', function(playerheading, playercoords, playerlocation)
	local x, y, z   = table.unpack(playercoords + playerlocation * 1.0)
	z = z - 1.0
	SetEntityCoords(GetPlayerPed(-1), x, y, z)
	SetEntityHeading(GetPlayerPed(-1), playerheading)
