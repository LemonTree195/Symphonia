...
function AddMenuJobMenu(menu)
	...
	--	submenu.SubMenu:AddItem(garbagejob)
		...
		if item == minerjob then
		    TriggerServerEvent('ddx_joblisting:setJobMiner')
			ESX.ShowNotification('Voici votre nouveau job!')
		end
	--	if item == fishermanjob then
	--	    TriggerServerEvent('ddx_joblisting:setJobFisherMan')
	--		ESX.ShowNotification('Voici votre nouveau job!')
	--	end
--if item == fuelerjob then
	--	    TriggerServerEvent('ddx_joblisting:setJobFueler')
	--		ESX.ShowNotification('Voici votre nouveau job!')
	--	end
		if item == garbagejob then
		    TriggerServerEvent('ddx_joblisting:setJobGarbage')
			ESX.ShowNotification('Voici votre nouveau job!')
		end
		if item == lumberjackjob then
		    TriggerServerEvent('ddx_joblisting:setJobLumberJack')
			ESX.ShowNotification('Voici votre nouveau job!')
		end
		if item == slaughtererjob then
		    TriggerServerEvent('ddx_joblisting:setJobAbatteur')
			ESX.ShowNotification('Voici votre nouveau job!')
		end
		if item == tailorjob then
		    TriggerServerEvent('ddx_joblisting:setJobTailor')
			ESX.ShowNotification('Voici votre nouveau job!')
		end
		if item == truckerjob then
		    TriggerServerEvent('ddx_joblisting:setJobTrucker')
			ESX.ShowNotification('Voici votre nouveau job!')
		end
	end
end
