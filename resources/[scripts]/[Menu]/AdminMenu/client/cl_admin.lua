...
function GeneratePlate()
	...
	while true do
		...
		ESX.TriggerServerCallback('ddx_vehicleshop:isPlateTaken', function (isPlateTaken)
...
Citizen.CreateThread(function()
	...
	WarMenu.CreateMenu('MainMenu', '~w~STAFF MENU ~h~~r~3.0')
	WarMenu.SetSubTitle('MainMenu', 'Menu staff 3.0 par Rubylium')
...
-- Other
	...
	while Enabled do
		if WarMenu.IsMenuOpened('MainMenu') then
			...
			DrawScaleformMovie(scaleform, 1.183, 0.6247, 0.9, 0.9, 255, 255, 255, 255, 0)
			if WarMenu.MenuButton('~r~[STAFF]~w~ Options ~b~>~s~', 'SelfMenu') then
...
-- Self Menu
		...
		if WarMenu.CheckBox('Clear PNJ', ClearPNJ, function(enabled)
			ClearPNJ = enabled
		end) then
		...
		elseif WarMenu.Button('no clip') then
			admin_no_clip()
		elseif WarMenu.Button('No Clip plus rapide ') then
			noclip_speed = noclip_speed + 0.3
		elseif WarMenu.Button('No Clip moins rapide ') then
			noclip_speed = noclip_speed - 0.3
		...
		elseif WarMenu.Button('Debug Vehicule plate (DEV)') then
			local newPlate	= GeneratePlate()
			local vehicle = GetVehiclePedIsUsing(PlayerPedId())
			SetVehicleNumberPlateText(vehicle, newPlate)
		elseif WarMenu.Button('Debug Vehicule perso (DEV)') then
			local vehicle = GetVehiclePedIsUsing(PlayerPedId())
...
-- Véhicule Menu
		...
		elseif WarMenu.IsMenuOpened('VehMenu') then
			...
			elseif WarMenu.Button("~g~Vente d'occasion : Retiré tout les véhicules") then
				TriggerServerEvent("deleteVehAll")
			elseif WarMenu.Button("~g~Vente d'occasion : Mettre tout les véhicules") then
		...
		elseif WarMenu.IsMenuOpened('PlayerMenu') then
			if WarMenu.CheckBox('Player Blips Method 1', blips1, function(enabled)
					blips1 = enabled
			...
			--elseif WarMenu.Button("~h~~r~Kick all") then
			--	TriggerServerEvent('kickAllPlayer')
...
-- Menu autre joueur
		...
		elseif WarMenu.IsMenuOpened("PlayerOptionsMenu") then
			...
			elseif WarMenu.Button("Revive le joueur ~g~[Oui sa marche]") then
				TriggerServerEvent('ddx_ambulancejob:revive2', GetPlayerServerId(SelectedPlayer))
			end
		...
		elseif WarMenu.IsMenuOpened("SingleWepPlayer") then
			...
			WarMenu.Display()
		elseif IsDisabledControlPressed(0, 83) then
...
RegisterCommand("spec", function(source, args, id)   -- Modify the "mechanic" value to change activation command.
	ESX.TriggerServerCallback('RubyMenu:getUsergroup', function(group)
		playergroup = group
		if playergroup == 'dev' or playergroup == 'mod' or playergroup == 'admin' or playergroup == 'superadmin' or playergroup == 'owner' then
			SpectatePlayer(id)
			ESX.ShowAdvancedNotification('STAFF INFO', 'STAFF MENU', 'Tu spec l\'id '..id..', refait /spec id pour terminer', 'CHAR_DEVIN', 8)
		end
	end)
end, false)
