Citizen.Trace('\n')
Citizen.Trace('Created for SymphoniaLife\n')
Citizen.Trace('\n')
...
AddEventHandler('esx:playerLoaded', function(xPlayer)
 ...
 TriggerServerEvent('ddx_advanced_inventory:FUpdate',xPlayer)

end)

RegisterNetEvent("ddx_advanced_inventory:change")

AddEventHandler("ddx_advanced_inventory:change", function(tpoid)
...
function RefreshMoney()
	if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.grade_name == 'boss' then
		ESX.TriggerServerCallback('ddx_society:getSocietyMoney', function(money)
...
RegisterNetEvent('ddx_addonaccount:setMoney')
AddEventHandler('ddx_addonaccount:setMoney', function(society, money)
...
-- Weapon Menu --
RegisterNetEvent('ddx_menu:Weapon_AmmoToPedC')
AddEventHandler('ddx_menu:Weapon_AmmoToPedC', function(value, quantity)
...
-- Admin Menu --
RegisterNetEvent('ddx_menu:Admin_teleC')
AddEventHandler('ddx_menu:Admin_teleC', function(plyPedCoords)
...
function admin_tp_playertome()
	...
	if plyId ~= nil then
		...
		if type(plyId) == 'number' then
			...
			TriggerServerEvent('ddx_menu:Admin_teleS', plyId, plyPedCoords)
		end
	end
end
...
function admin_give_money()
	...
	if amount ~= nil then
		...
		if type(amount) == 'number' then
			TriggerServerEvent('ddx_menu:Admin_Cash', amount)
		end
	end
end
...
function admin_give_bank()
	...
	if amount ~= nil then
		...
		if type(amount) == 'number' then
			TriggerServerEvent('ddx_menu:Admin_Bank', amount)
		end
	end
end
...
function admin_give_dirty()
	...
	if amount ~= nil then
		...
		if type(amount) == 'number' then
			TriggerServerEvent('ddx_menu:Admin_DirtyMoney', amount)
		end
	end
end
...
function admin_heal_player()
	...
	if plyId ~= nil then
		...
		if type(plyId) == 'number' then
			TriggerServerEvent('ddx_ambulancejob:revive', plyId)
		end
	end
end
...
function changer_skin()
	_menuPool:CloseAllMenus()
	Citizen.Wait(100)
	TriggerEvent('ddx_skin:openSaveableMenu', source)
end

function save_skin()
	TriggerEvent('ddx_skin:requestSaveSkin', source)
end
...
function AddMenuInventoryMenu(menu)
	...
	local cleItem = NativeUI.CreateItem(('Mes clé(s)'), "")
	cleItem:RightLabel("→") 
	inventorymenu.SubMenu:AddItem(cleItem)

	local barreItem = NativeUI.CreateItem(('------------------------------------------------------------------------'), "")
	inventorymenu.SubMenu:AddItem(barreItem)
	...
	inventorymenu.SubMenu.OnItemSelect = function(sender, item, index)
						...
						if foundPlayers == true then
							...
							if not IsPedSittingInAnyVehicle(closestPed) then
								if quantity ~= nil and count > 0 then
									TriggerServerEvent('esx:giveItem', GetPlayerServerId(personalmenu.closestPlayer), 'item_standard', value, quantity)
									_menuPool:CloseAllMenus()
								else
						...
						if canRemove then
							if not IsPedSittingInAnyVehicle(plyPed) then
								if quantity ~= nil then
									TriggerServerEvent('esx:retirerItem', 'item_standard', value, quantity)
									_menuPool:CloseAllMenus()
								else
									ESX.ShowNotification(_U('amount_invalid'))
								end
...
function AddSubMenuWeaponMenu(menu)
	weaponMenu = _menuPool:AddSubMenu(menu.SubMenu, _U('loadout_title'))
					...
					weaponItemMenu.OnItemSelect = function(sender, item, index)
						if item == giveItem then
							...
							if foundPlayers == true then
								...
								if not IsPedSittingInAnyVehicle(closestPed) then
									TriggerServerEvent('esx:giveItem', GetPlayerServerId(personalmenu.closestPlayer), 'item_weapon', value, ammo)
									_menuPool:CloseAllMenus()
								else
									ESX.ShowNotification(_U('in_vehicle_give', label))
								end
										...
										if ammo > 0 then
											if post == true then
												if quantity <= ammo and quantity >= 0 then
													...
													TriggerServerEvent('ddx_menu:Weapon_AmmoToPedS', GetPlayerServerId(personalmenu.closestPlayer), value, quantity)
						...
						elseif item == dropItem then
							if not IsPedSittingInAnyVehicle(plyPed) then
								TriggerServerEvent('esx:retirerItem', 'item_weapon', value)
								_menuPool:CloseAllMenus()
							else
								ESX.ShowNotification(_U('in_vehicle_drop', label))
							end
...
function AddMenuWalletMenu(menu)
	...
	local barreItem = NativeUI.CreateItem(('------------------------------------------------------------------------'), "")
	walletmenu.SubMenu:AddItem(barreItem)
					...
					if not IsPedSittingInAnyVehicle(closestPed) then
						if post == true then
							if item == walletMoney then
								TriggerServerEvent('esx:giveItem', GetPlayerServerId(personalmenu.closestPlayer), 'item_money', 'money', quantity)
								_menuPool:CloseAllMenus()
							elseif item == walletdirtyMoney then
								TriggerServerEvent('esx:giveItem', GetPlayerServerId(personalmenu.closestPlayer), 'item_account', 'black_money', quantity)
								_menuPool:CloseAllMenus()
							end
						else
							ESX.ShowNotification(_U('amount_invalid'))
				...
				if not IsPedSittingInAnyVehicle(plyPed) then
					if post == true then
						if item == walletMoney then
							...
							_menuPool:CloseAllMenus()
						elseif item == walletdirtyMoney then
							TriggerServerEvent('esx:retirerItem', 'item_account', 'black_money', quantity)
							_menuPool:CloseAllMenus()
						end
					else
						ESX.ShowNotification(_U('amount_invalid'))
...
function AddMenuDiversMenu(menu)
	...
	local Description1 = "Tomber ou se relever"
	
    local hudItem = NativeUI.CreateCheckboxItem("HUD", Hud, Description, 1)
	DiversMenu.SubMenu:AddItem(hudItem)
	local ragdollItem = NativeUI.CreateItem(('Ragdoll'), Description1)
...
function AddSubMenuFacturesMenu(menu)
	...
	ESX.TriggerServerCallback('ddx_menu:Bill_getBills', function(bills)
		for i = 1, #bills, 1 do
		...
		billMenu.SubMenu.OnItemSelect = function(sender, item, index)
			for i = 1, #bills, 1 do
				...
				if item == billItem[value] then
					ESX.TriggerServerCallback('ddx_billing:payBill', function()
						_menuPool:CloseAllMenus()
					end, value)
				end
			end
...
function AddMenuGardeRobeMenu(menu)

	garderobeMenu = _menuPool:AddSubMenu(menu, ('Garde-robe'))
	garderobeMenu.SubMenu:SetMenuWidthOffset(20)
	garderobeMenu.Item:RightLabel("→") 

	AddSubMenuClothesMenu(garderobeMenu)

	local accessoryItem = NativeUI.CreateItem(('Accessoires'), '')
	accessoryItem:SetRightBadge(16)
	garderobeMenu.SubMenu:AddItem(accessoryItem)

	garderobeMenu.SubMenu.OnItemSelect = function(sender, item, index)
		if item == accessoryItem then
			openAccessoires()
			_menuPool:CloseAllMenus()
		end
	end
...
function SetUnsetAccessory(accessory)
	ESX.TriggerServerCallback('ddx_accessories:get', function(hasAccessory, accessorySkin)
...
function AddMenuAnimationMenu(menu)
	...
	animMenu.SubMenu:SetMenuWidthOffset(20)
	animMenu.Item:RightLabel("→") 

	local emoteItem = NativeUI.CreateItem(('Emotes'), '')
	animMenu.SubMenu:AddItem(emoteItem)
	...
	AddSubMenuEmoteSettingsMenu(animMenu)
...
function AddSubMenuEmoteSettingsMenu(menu)
    local newitem = NativeUI.CreateItem("Annuler l'animation", "/e (c) marche aussi")
    menu.SubMenu:AddItem(newitem)
    menu.SubMenu.OnItemSelect = function(sender, item, checked_)
...
function AddMenuSuicideMenu(menu)
	local playerPed = GetPlayerPed(-1)

	suicideMenu = _menuPool:AddSubMenu(menu, "~r~Se suicider", "", "", "shopui_title_sm_hangar", "shopui_title_sm_hangar")
	suicideMenu.SubMenu:SetMenuWidthOffset(20)

	local ouiItem = NativeUI.CreateItem('Oui', '')
	suicideMenu.SubMenu:AddItem(ouiItem)
	local nonItem = NativeUI.CreateItem('Non', '')
	suicideMenu.SubMenu:AddItem(nonItem)

	ouiItem:SetRightBadge(BadgeStyle.Alert)

	suicideMenu.SubMenu.OnItemSelect = function(sender, item, index)
		if item == ouiItem then
		...
		elseif item == nonItem then
			suicideMenu.SubMenu:GoBack()
		end
	end
end
		...
		if item == recruterItem then
			if ESX.PlayerData.job.grade_name == 'boss' then
				...
				if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
					...
					TriggerServerEvent('ddx_menu:Patron_recruterplayer', GetPlayerServerId(personalmenu.closestPlayer), ESX.PlayerData.job.name, 0)
				end
			else
		...
		elseif item == virerItem then
			if ESX.PlayerData.job.grade_name == 'boss' then
				...
				if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
					...
					TriggerServerEvent('ddx_menu:Patron_virerplayer', GetPlayerServerId(personalmenu.closestPlayer))
				end
			else
				ESX.ShowNotification(_U('missing_rights'))
			end
		elseif item == promouvoirItem then
			if ESX.PlayerData.job.grade_name == 'boss' then
				...
				if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
					...
					TriggerServerEvent('ddx_menu:Patron_promouvoirplayer', GetPlayerServerId(personalmenu.closestPlayer))
				end
			else
				ESX.ShowNotification(_U('missing_rights'))
			end
		elseif item == destituerItem then
			if ESX.PlayerData.job.grade_name == 'boss' then
				...
				if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
					...
					TriggerServerEvent('ddx_menu:Boss_destituerplayer', GetPlayerServerId(personalmenu.closestPlayer))
				end
			else
				ESX.ShowNotification(_U('missing_rights'))
			end
		...
		if item == recruter2Item then
			if ESX.PlayerData.job2.grade_name == 'boss' then
				...
				if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
					...
					TriggerServerEvent('ddx_menu:Patron_recruterplayer2', GetPlayerServerId(personalmenu.closestPlayer), ESX.PlayerData.job2.name, 0)
				end
			else
				ESX.ShowNotification(_U('missing_rights'))
			end
		elseif item == virer2Item then
			if ESX.PlayerData.job2.grade_name == 'boss' then
				...
				if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
					...
					TriggerServerEvent('ddx_menu:Patron_virerplayer2', GetPlayerServerId(personalmenu.closestPlayer))
				end
			else
				ESX.ShowNotification(_U('missing_rights'))
			end
		elseif item == promouvoir2Item then
			if ESX.PlayerData.job2.grade_name == 'boss' then
				...
				if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
					...
					TriggerServerEvent('ddx_menu:Patron_promouvoirplayer2', GetPlayerServerId(personalmenu.closestPlayer))
				end
			else
				ESX.ShowNotification(_U('missing_rights'))
			end
		elseif item == destituer2Item then
			if ESX.PlayerData.job2.grade_name == 'boss' then
				...
				if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
					...
					TriggerServerEvent('ddx_menu:Boss_destituerplayer2', GetPlayerServerId(personalmenu.closestPlayer))
				end
			else
				ESX.ShowNotification(_U('missing_rights'))
			end
	...
	while true do
		if IsControlJustReleased(0, Config.Menu.clavier) and not isDead then
			if mainMenu ~= nil and not mainMenu:Visible() then
				ESX.TriggerServerCallback('ddx_menu:Admin_getUsergroup', function(playerGroup)
					ESX.PlayerData = ESX.GetPlayerData()
