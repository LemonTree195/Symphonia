...
function updateIdentity(playerId, data, callback)
	...
	MySQL.Async.execute('UPDATE `users` SET `firstname` = @firstname, `lastname` = @lastname, `dateofbirth` = @dateofbirth, `sex` = @sex, `height` = @height WHERE identifier = @identifier', {
		...
		if callback then
			TriggerEvent('ddx_identity:characterUpdated', playerId, data)
			callback(true)
		end
	end)
end
...
RegisterServerEvent('ddx_identity:setIdentity')
AddEventHandler('ddx_identity:setIdentity', function(data, myIdentifiers)
	local xPlayer = ESX.GetPlayerFromId(source)
	setIdentity(myIdentifiers.steamid, data, function(callback)
		if callback then
			TriggerClientEvent('ddx_identity:identityCheck', myIdentifiers.playerid, true)
			TriggerEvent('ddx_identity:characterUpdated', myIdentifiers.playerid, data)
		else
...
AddEventHandler('esx:playerLoaded', function(playerId, xPlayer)
	...
	TriggerClientEvent('ddx_identity:saveID', playerId, myID)

	getIdentity(playerId, function(data)
		if data.firstname == '' then
			TriggerClientEvent('ddx_identity:identityCheck', playerId, false)
			TriggerClientEvent('ddx_identity:showRegisterIdentity', playerId)
		else
			TriggerClientEvent('ddx_identity:identityCheck', playerId, true)
			TriggerEvent('ddx_identity:characterUpdated', playerId, data)
		end
	end)
end)

AddEventHandler('ddx_identity:characterUpdated', function(playerId, data)
	local xPlayer = ESX.GetPlayerFromId(playerId)
	...
	if resource == GetCurrentResourceName() then
		...
		for i=1, #xPlayers, 1 do
			...
			if xPlayer then
				...
				TriggerClientEvent('ddx_identity:saveID', xPlayer.source, myID)
	
				getIdentity(xPlayer.source, function(data)
					if data.firstname == '' then
						TriggerClientEvent('ddx_identity:identityCheck', xPlayer.source, false)
						TriggerClientEvent('ddx_identity:showRegisterIdentity', xPlayer.source)
					else
						TriggerClientEvent('ddx_identity:identityCheck', xPlayer.source, true)
						TriggerEvent('ddx_identity:characterUpdated', xPlayer.source, data)
					end
				end)
			end
		end
...
TriggerEvent('es:addCommand', 'register', function(source, args, user)
	getCharacters(source, function(data)
	  if data.firstname3 ~= '' then
		...
		TriggerClientEvent('ddx_identity:showRegisterIdentity', source, {})
	  end
	end)
  end)