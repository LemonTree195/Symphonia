...
RegisterNetEvent('ddx_identity:showRegisterIdentity')
AddEventHandler('ddx_identity:showRegisterIdentity', function()
	if not isDead then
		EnableGui(true)
	end
end)

RegisterNetEvent('ddx_identity:identityCheck')
AddEventHandler('ddx_identity:identityCheck', function(identityCheck)
	hasIdentity = identityCheck
end)

RegisterNetEvent('ddx_identity:saveID')
AddEventHandler('ddx_identity:saveID', function(data)
	myIdentifiers = data
end)
...
RegisterNUICallback('register', function(data, cb)
	...
	if reason == "" then
		TriggerServerEvent('ddx_identity:setIdentity', data, myIdentifiers)
		EnableGui(false)
		Citizen.Wait(500)
		TriggerEvent('ddx_skin:openSaveableMenu', myIdentifiers.id)
	else
