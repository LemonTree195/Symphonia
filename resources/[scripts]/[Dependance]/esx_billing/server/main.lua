...
RegisterServerEvent('ddx_billing:sendBill24')
AddEventHandler('ddx_billing:sendBill24', function(playerId, sharedAccountName, label, amount)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local xTarget = ESX.GetPlayerFromId(playerId)

	TriggerEvent('ddx_addonaccount:getSharedAccount', sharedAccountName, function(account)

		if amount < 0 then
			print('ddx_billing: ' .. GetPlayerName(_source) .. ' tried sending a negative bill!')
...
ESX.RegisterServerCallback('ddx_billing:getBills', function(source, cb)
...
ESX.RegisterServerCallback('ddx_billing:getTargetBills', function(source, cb, target)
...
ESX.RegisterServerCallback('ddx_billing:payBill', function(source, cb, id)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.fetchAll(
		'SELECT * FROM billing WHERE id = @id',
		{
			...
			if targetType == 'player' then
				...
				TriggerEvent('ddx_addonaccount:getSharedAccount', target, function(account)
