...
AddEventHandler('trew_hud_ui:setInfo', function(info)
	...
	SendNUIMessage({ action = 'setText', id = 'job2', value = info['job2'] })
	...
	if ESX.PlayerData.job ~= nil then
		if ESX.PlayerData.job.grade_name ~= nil and ESX.PlayerData.job.grade_name == 'boss' then
			...
			end, ESX.PlayerData.job.name and ESX.PlayerData.job2.name)
		else
...
RegisterNetEvent('trew_hud_ui:hud')
AddEventHandler('trew_hud_ui:hud', function()
		...
		while true do
			...
			if IsControlJustPressed(1, Keys[Config.voice.keys.distance]) then
				...
				elseif Config.voice.levels.current == 2 then
					...
					ShowNotif("Le niveau de votre voix a été réglé sur ~y~chuchoter")
				end
...
-- The watermark text --
servername = "~w~SymphoniaLife - discord.gg/fYvMAxh"

-- The x and y offset (starting at the top left corner) --
-- Default: 0.005, 0.001
offset = {x = 0.495, y = 0.001}
