...
function takeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Tu as besoin d'une arme pour prendre en hotage!")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= -1 and closestPlayer ~= nil then
			...
			drawNativeNotification("Personne à proximité pour prendre en otage !")
		end 
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Appuyez sur [G] libérer, [H] pour tuer")
