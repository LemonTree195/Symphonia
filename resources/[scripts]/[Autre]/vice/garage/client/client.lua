...
function ListVehiclesMenu()
    ...
    ESX.TriggerServerCallback('ddx_garage:getVehicles', function(vehicles)
...
function reparation(prix, vehicle, vehicleProps)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'delete_menu', {
        ...
        if (data.current.value == 'yes') then
            TriggerServerEvent('ddx_garage:payhealth', prix)
...
RegisterNetEvent('ddx_garage:deletevehicle_cl')

AddEventHandler('ddx_garage:deletevehicle_cl', function(plate)
...
function ranger(vehicle, vehicleProps)
    TriggerServerEvent('ddx_garage:deletevehicle_sv', vehicleProps.plate)
    TriggerServerEvent('ddx_garage:modifystate', vehicleProps, true)
...
function StockVehicleMenu()
    ...
    if IsPedInAnyVehicle(playerPed, false) then
        ...
        ESX.TriggerServerCallback('ddx_garage:stockv', function(valid)
            if (valid) then
                ESX.TriggerServerCallback('ddx_garage:getVehicles', function(vehicules)
                    ...
                    for _, v in pairs(vehicules) do
                        if plate == v.plate then
                            owned = true
                            TriggerServerEvent('ddx_garage:debug', 'vehicle plate returned to the garage: ' .. vehicleProps.plate)
                            TriggerServerEvent('ddx_garage:logging', 'vehicle returned to the garage: ' .. engineHealth)
...
function SpawnVehicle(vehicle)
    ...
    TriggerServerEvent('ddx_garage:modifystate', vehicle, false)
end
...
function SpawnPoundedVehicle(vehicle)
    ...
    TriggerServerEvent('ddx_garage:modifystate', vehicle, true)
...
-- Marker actions
AddEventHandler('ddx_garage:hasEnteredMarker', function(zone)
    if zone == 'spawn' then
        CurrentAction = 'spawn'
...
AddEventHandler('ddx_garage:hasExitedMarker', function(zone)
    ESX.UI.Menu.CloseAll()
    CurrentAction = nil
end)

function ReturnVehicleMenu()
    ESX.TriggerServerCallback('ddx_garage:getOutVehicles', function(vehicles)
        ...
        for _,v in pairs(vehicles) do
            ...
            labelvehicle = vehicleName..': '..('Retour')
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_vehicle', {
            ...
            ESX.TriggerServerCallback('ddx_garage:checkMoney', function(hasEnoughMoney)
                if hasEnoughMoney then
                    if times == 0 then
                        TriggerServerEvent('ddx_garage:pay')
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if isInMarker and not hasAlreadyEnteredMarker then
            ...
            TriggerEvent('ddx_garage:hasEnteredMarker', currentZone)
        end

        if not isInMarker and hasAlreadyEnteredMarker then
            hasAlreadyEnteredMarker = false
            TriggerEvent('ddx_garage:hasExitedMarker', LastZone)
            RenderScriptCams(0, 1, 2000, 1, 1)
            DestroyCam(camera, true)
        end
    end
