...
TriggerEvent('es:addCommand', 'report', function(source, args, user)
    ...
    if reason=="" or not reason then TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","spécifiez une raison"}}); return end
        ...
        if ac>0 then
            ...
            Citizen.SetTimeout(120000,function()
                ...
                if GetPlayerName(source)~=nil then
                    TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","Votre ticket a expirer"}})
                end
            end)
            TriggerClientEvent("chat:addMessage",source,{color={0,255,0},multiline=false,args={"BWH","Ticket (expire dans 120s), ecrivez ^1/aticket^7 pour annulé votre ticket"}})
        else
            TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","Il n'y a pas d'admin sur le serveur"}})
        end
    else
        TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","Quelq'un vous aide deja ou vous avez un ticket en cours"}})
    end
end)

TriggerEvent('es:addCommand', 'aticket', function(source, args, user)
...
TriggerEvent('es:addCommand', 'return', function(source, args, user)
    local xPlayer = ESX.GetPlayerFromId(source)
    if isAdmin(xPlayer) then
        local found = false
        for k,v in pairs(active_assists) do
            if v==source then
                found = true
                active_assists[k]=nil
                TriggerClientEvent("chat:addMessage",source,{color={0,255,0},multiline=false,args={"BWH","Ticket fermer vous allez etre tp"}})
                TriggerClientEvent("el_bwh:assistDone",source)
            end
        end
        if not found then TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","Vous aidez personne"}}) end
    else
        TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","tu n'a pas les permissions pour faire cette commande!"}})
    end
end)
...
function acceptAssist(xPlayer,target)
    if isAdmin(xPlayer) then
        ...
        for k,v in pairs(active_assists) do
            if v==source then
                TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","Tu aide deja une personne"}})
                return
            end
        end
        if open_assists[target] and not active_assists[target] then
            ...
            TriggerClientEvent("chat:addMessage",source,{color={0,255,0},multiline=false,args={"BWH","Teleportement sur le joueur..."}})
        elseif not open_assists[target] and active_assists[target] and active_assists[target]~=source then
            TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","Quelq'un aide deja ce joueur"}})
        else
            TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","Ce joueur ne demande pas d'aide"}})
        end
    else
        TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","Tu n'a pas les permissions pour faire cette commande!"}})
    end
