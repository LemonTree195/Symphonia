...
AddEventHandler("el_bwh:receiveWarn",function(sender,message)
	TriggerEvent("chat:addMessage",{color={255,255,0},multiline=true,args={"BWH","Vous avez recu un avertissement"..(sender~="" and " de "..sender or "").."!\n-> "..message}})
	Citizen.CreateThread(function()
		local scaleform = RequestScaleformMovie("mp_big_message_freemode")
		while not HasScaleformMovieLoaded(scaleform) do Citizen.Wait(0) end
		BeginScaleformMovieMethod(scaleform, "SHOW_SHARD_WASTED_MP_MESSAGE")
		PushScaleformMovieMethodParameterString("~y~Avertissement")
...
RegisterCommand("r",function(a,b,c)
...
TriggerEvent('chat:addSuggestion', '/r', 'Enlever la notif d\'un ticket',{})
TriggerEvent('chat:addSuggestion', '/report', 'Faire un ticket',{{name="Reason", help="Pourquoi avez vous besoin d'aide?"}})
TriggerEvent('chat:addSuggestion', '/aticket', 'Annulé son ticket',{})
TriggerEvent('chat:addSuggestion', '/return', 'Finir le ticket et ce tp',{})
TriggerEvent('chat:addSuggestion', '/a', 'Accepter le ticket', {{name="ID du joueur", help="Id du joueur que tu veux aider"}})