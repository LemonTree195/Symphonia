...
function OpenAdminActionMenu(player)
    ...
    ESX.TriggerServerCallback('ddx_spectate:getOtherPlayerData', function(data)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(1, 163) then
			print('triggered')
			if group ~= "user" then
				TriggerEvent('ddx_spectate:spectate')
			end
		end
	end
end)
...
RegisterNetEvent('ddx_spectate:spectate')
AddEventHandler('ddx_spectate:spectate', function()
...
RegisterNUICallback('kick', function(data, cb)
	SetNuiFocus(false)
	TriggerServerEvent('ddx_spectate:kick', data.id, data.reason)
	TriggerEvent('ddx_spectate:spectate')
end)
