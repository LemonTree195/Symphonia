	...
	TriggerEvent('ddx_skin:openRestrictedMenu', function(data, menu)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data.current.value == 'yes' then
				ESX.TriggerServerCallback('ddx_barbershop:checkMoney', function(hasEnoughMoney)
					if hasEnoughMoney then
						TriggerEvent('skinchanger:getSkin', function(skin)
							TriggerServerEvent('ddx_skin:save', skin)
						end)

						TriggerServerEvent('ddx_barbershop:pay')
						hasPaid = true
					else
...
AddEventHandler('ddx_barbershop:hasEnteredMarker', function(zone)
	currentAction = 'shop_menu'
	currentActionMsg = _U('press_access')
end)

AddEventHandler('ddx_barbershop:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not hasAlreadyEnteredMarker) or (isInMarker and lastZone ~= currentZone) then
			...
			TriggerEvent('ddx_barbershop:hasEnteredMarker', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('ddx_barbershop:hasExitedMarker', lastZone)
		end
