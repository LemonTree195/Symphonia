...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	ESX.TriggerServerCallback('ddx_lscustom:getVehiclesPrices', function(vehicles)
...
RegisterNetEvent('ddx_lscustom:installMod')

AddEventHandler('ddx_lscustom:installMod', function()

	myCar = ESX.Game.GetVehicleProperties(GetVehiclePedIsIn(PlayerPedId(), false))

	TriggerServerEvent('ddx_lscustom:refreshOwnedVehicle', myCar)

end)



RegisterNetEvent('ddx_lscustom:cancelInstallMod')

AddEventHandler('ddx_lscustom:cancelInstallMod', function()
...
function OpenLSMenu(elems, menuname, menutitle, parent)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuname, {
		...
		for k, v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.rightlabel[1], _U('installed')) then
					...
					TriggerEvent('ddx_lscustom:installMod')
					...
					if isRimMod then
						...
						TriggerServerEvent("ddx_lscustom:buyMod", price)
		...
		if isBuying == nil then
			...
			TriggerEvent('ddx_lscustom:cancelInstallMod')
