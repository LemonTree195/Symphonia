...
ESX.RegisterServerCallback('eden_garage:getVehicles', function(source, cb)
	...
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE owner=@identifier",{['@identifier'] = xPlayer.getIdentifier()}, function(data) 
		for _,v in pairs(data) do
			...
			table.insert(vehicules, {vehicle = vehicle, state = v.state, plate = v.plate, garageperso = v.garageperso})
		end
		cb(vehicules)
	end)
end)
...
AddEventHandler('eden_garage:pay', function()
	...
	local xPlayer = ESX.GetPlayerFromId(source)
	local total   = (Config.Price)



    TriggerEvent('ddx_addonaccount:getSharedAccount', 'society_mecano', function(account)
      account.addMoney(total)
    end)
	
	xPlayer.removeMoney(Config.Price)
    --xPlayer.getBank(money)
...
AddEventHandler('eden_garage:payhealth', function(price)
	local xPlayer = ESX.GetPlayerFromId(source)
    --local balance = xPlayer.setBankBalance()
    local total   = (price)
