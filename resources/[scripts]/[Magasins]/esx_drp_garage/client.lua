...
Citizen.CreateThread(function()
	  while true do Citizen.Wait(0)
		...
		HealthBodyVeh         = GetVehicleBodyHealth(MyPedVeh)
		local TyreBurst1      = IsVehicleTyreBurst(MyPedVeh, 0)
				local TyreBurst2      = IsVehicleTyreBurst(MyPedVeh, 1)
				local TyreBurst3      = IsVehicleTyreBurst(MyPedVeh, 4)
		local TyreBurst4      = IsVehicleTyreBurst(MyPedVeh, 5)
			if (TyreBurst1 or TyreBurst2 or TyreBurst3 or TyreBurst4) then
				TyreBurst = false
			else
				TyreBurst = true
			end      
		end
		if (HealthBodyVeh < 950 or TyreBurst == false) then
			etat = 'HS'
		else
			etat = 'OK'
		end
...
RegisterNetEvent('cyprom:etatVeh')
AddEventHandler('cyprom:etatVeh', function(cb)
	cb(etat)
end)
...
function ListVehiclesMenu()
	...
	ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
		...
		for _,v in pairs(vehicles) do
    		...
    		local labelvehicle
    		if(v.state) and not v.garageperso then
    		labelvehicle = _U('status_in_garage', GetLabelText(vehicleName))	
    		else
    		    if v.garageperso then 
    		    	labelvehicle = _U('status_other_garage', GetLabelText(vehicleName))
    		    	impound = false
    		    else	 
		...
		function(data, menu)
			if(data.current.value.state)then
				...
				if data.current.value.garageperso then
					menu.close()
				    ESX.ShowNotification('~r~Le véhicule n\'est pas disponible dans ce garage')
				else 
...
function SpawnVehicle(vehicle)
	...
	if not IsAnyVehicleNearPoint(this_Garage.SpawnPoint.Pos.x, this_Garage.SpawnPoint.Pos.y, this_Garage.SpawnPoint.Pos.z, 5.0) then
		...
		ESX.ShowNotification('Il y a déja un véhicule de sorti, patientez...')	end
