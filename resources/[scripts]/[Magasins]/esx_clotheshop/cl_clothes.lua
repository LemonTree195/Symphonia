...
function ClotheShopAdd(menu)
	...
	ESX.TriggerServerCallback('ddx_skin:getPlayerSkin', function(skin, _)
		curSex = skin.sex
...
function sacFct(menu)
	playerPed = GetPlayerPed(-1)
	n = NativeUI.CreateItem("Sac à dos","")
	menu:AddItem(n)
	c = NativeUI.CreateItem("Sac tactique","")
	menu:AddItem(c)
	cx = NativeUI.CreateItem("Aucun sac","")
	menu:AddItem(cx)
	menu.OnItemSelect = function (_, _, index)
		if index == 1 then
			SetPedComponentVariation(playerPed,7, 3, 0, 2)
			TriggerEvent('skinchanger:getSkin', function(skin)
...
Citizen.CreateThread(function()
    ...
    for i = 1, #ConfigclotheShop.Map, 1 do
        ...
        AddTextComponentString('Suburban')
...
function maskMenu(menu)
    local accessories = { "Masque", "Chapeau", "Lunette", "Boucles d'oreille", "Gilet par balles", "Sac a dos" }
    local accessoriesIndex = { "mask", "hat", "glasses", "ears", "gilet", "sac" }
...
RegisterNetEvent('NB:accessoires')
AddEventHandler('NB:accessoires', function()
	_menuPool:CloseAllMenus()
	OpenAccessMenus()
end)
